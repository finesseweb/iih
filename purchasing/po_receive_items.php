...
function display_po_receive_items()
{
    ...
    $th = array(_("Item Code"), _("Description"), _("Ordered"), _("Units"), _("Received"),
    	_("Outstanding"), _("This Delivery"), _("Price"), _("GST"), _("IST"), _("CST"), _("Total"));
    table_header($th);
    ...
    if (count($_SESSION['PO']->line_items)> 0 )
    {
       	foreach ($_SESSION['PO']->line_items as $ln_itm)
       	{
    		...
    		$line_total = ($ln_itm->receive_qty * ($ln_itm->price+$ln_itm->gst+$ln_itm->ist+$ln_itm->cst));
    		$total += $line_total;
			...
			amount_decimal_cell($ln_itm->price);
                        amount_decimal_cell($ln_itm->gst);
                        amount_decimal_cell($ln_itm->ist);
                        amount_decimal_cell($ln_itm->cst);
