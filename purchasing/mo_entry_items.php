...
page(_($help_context = "Issue Items to Floor"), false, false, "", $js);
...
$cart = &$_POST;
$failed_data = add_mo();
///display_error($failed_data);die();
///	$failed_data = add_po($_POST['floor'],
//		$_POST['ref'], 0, $_SESSION['issue_items']->line_items,
//		$_POST['Location'], $_POST['WorkCentre'], $_POST['date_'], $_POST['memo_'],$_POST['sl_no'],"F");
...
if ($editable)
			{
				...
				label_row(_("Reference:"),"MO-".$reference);
...
echo "<br>";
end_outer_table(); 
//display_search_details('F');

div_start('controls', 'items_table');

//display_floor_requirements($_POST['floor']);
div_end();
$Ajax->activate('items_table');
echo "<br>";
start_table(TABLESTYLE, "width='90%'", 10);
echo "<tr><td>";
display_mo_items(_("Items to Issue"), $_SESSION['issue_items'],$_POST['ref_no']);
...
end_table();
echo "<br>";
submit_center('addissue', _("ADD Stock"), true, '', 'default');
