...
display_heading(_("Help Desk ID") . " #$help_id");

start_table(TABLESTYLE, "width='30%',align='left'");
echo"</br>";
start_row();
label_cell(_("Date"),"class='tableheader',width='20%'");
label_cell(dates2sql($from_trans[1]));
end_row();
start_row();
label_cell(_("Category"), "class='tableheader'");
label_cell($from_trans[12]);
end_row();

start_row();
label_cells(_("Name"), $from_trans[13], "class='tableheader'", "align=left");
end_row();


start_row();
label_cells(_("Maintenance Department"), $from_trans[14], "class='tableheader'", "align=left");
end_row();
start_row();
label_cells(_("Issues"), $from_trans[8], "class='tableheader'");
end_row();
start_row();
if(($from_trans[10] == 1) || ($from_trans[10] == 0) ){
label_cells(_("Status"), 'Pending', "class='tableheader'");
}
else{
label_cells(_("Status"), 'Completed', "class='tableheader'");
}

end_row();

comments_display_row(SA_HELPDESKVIEW, $help_id);
