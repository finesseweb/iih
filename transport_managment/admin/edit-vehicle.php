...
if (!@$_GET['popup'])
	page(_($help_context = "Edit Vehicle"));
if(strlen($_SESSION["wa_current_user"]->email)==0){
	...
	if(isset($_POST['submit'])) {
			//echo "<script>alert('sfgsgdh')</script>";
			//echo "<pre>";
			//print_r($_POST);
			...
			$sql="update tblvehicles set VehiclesTitle=:vehicletitle,VehiclesBrand=:brand,VehiclesOverview=:vehicleoverview,FuelType=:fueltype,ModelYear=:modelyear,SeatingCapacity=:seatingcapacity,AirConditioner=:airconditioner,PowerDoorLocks=:powerdoorlocks,AntiLockBrakingSystem=:antilockbrakingsys,BrakeAssist=:brakeassist,PowerSteering=:powersteering,DriverAirbag=:driverairbag,PassengerAirbag=:passengerairbag,PowerWindows=:powerwindow,CDPlayer=:cdplayer,CentralLocking=:centrallocking,CrashSensor=:crashcensor,LeatherSeats=:leatherseats,DriverId=:driver where id=:id ";
...
<head>
	...
	<style>
		...
		.col-sm-4.vehicle-botton button {
			padding: 3px;
		}
		.hr-dashed {
		...
		<div class="container-fluid">
			...
			<form action=""  method="POST" class="form-horizontal" name="form1">
				<div class="form-group">
					<label class="col-sm-2">Vehicle Title<span style="color:red">*</span></label>
					<div class="col-sm-4">
						<input type="text" name="vehicletitle" class="" value="<?php echo htmlentities($result->VehiclesTitle)?>" required>
					</div>
					<label class="col-sm-2">Select Brand<span style="color:red">*</span></label>
					<div class="col-sm-4 vehicle-botton">
				...
				<div class="form-group">
					<label class="col-sm-2">Vehical Overview<span style="color:red">*</span></label>
					<div class="col-sm-4 vehicle-botton">
						<textarea class="form-control" name="vehicalorcview" rows="3" required><?php echo htmlentities($result->VehiclesOverview);?></textarea>
					</div>
					<label class="col-sm-2">Select Fuel Type<span style="color:red">*</span></label>
					<div class="col-sm-4 vehicle-botton">
				...
				<div class="form-group">
					<label class="col-sm-1">Model Year<span style="color:red">*</span></label>
					<div class="col-sm-2">
						<input type="text" name="modelyear" class="" value="<?php echo htmlentities($result->ModelYear);?>" required>
					</div>
					<label class="col-sm-2">Seating Capacity<span style="color:red">*</span></label>
					<div class="col-sm-2">
						<input type="text" name="seatingcapacity" class="" value="<?php echo htmlentities($result->SeatingCapacity);?>" required>
					</div>
					<label class="col-sm-1">Assign Driver<span style="color:red">*</span></label>
					<div class="col-sm-4 vehicle-botton">
						<select class="selectpicker" name="driver" required>
						<?php 
						$id=$result->DriverId ;
						$sql ="SELECT name from driver_details where id=:id";
						$query = $dbh -> prepare($sql);
						$query-> bindParam(':id', $id, PDO::PARAM_STR);
						$query->execute();
						$driver_name=$query->fetchAll(PDO::FETCH_OBJ);
						?>
						<option value="<?= $id;?>"><?= $driver_name[0]->name;?></option>
						<?php $ret="select id,name from driver_details";
						$query= $dbh -> prepare($ret);
						//$query->bindParam(':id',$id, PDO::PARAM_STR);
						$query-> execute();
						$result12 = $query -> fetchAll(PDO::FETCH_OBJ);
						if($query -> rowCount() > 0){
				...
				<div class="form-group">
					<div class="col-sm-4">
						...
						<a href="<?php echo $path_to_root ?>/transport_managment/admin/changeimage1.php?imgid=<?php echo htmlentities($result->id)?>">Change Image 1</a>
					</div>
				</div>
				<div class="hr-dashed"></div>									
							</div>
			...
			<div class="row">
				...
				<div class="form-group">
					<div class="col-sm-8 col-sm-offset-2" style="text-align:center">
						<button type="submit" name="submit" class="btn btn-primary btn-sm" style="margin-top:4%">Save changes</button>
					</div>
				</div>
			</form>
		</div>
