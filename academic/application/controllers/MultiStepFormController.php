...
class MultiStepFormController extends Zend_Controller_Action {
    ...
    public function indexAction() {
       ...
       // if(md5($_SESSION['user_login']['user_login']['app_id'])!==$application_id){
        ...
        switch ($type) {
            ...
            case "step1":
                ...
                if($application_id){
                   ...
                   $this->view->department = $followUpData['course'];
                    $this->view->course = $followUpData['course'];
					 $this->view->session = $followUpData['session'];
					  $this->view->year = $followUpData['acad_year_id'];
					$populateData['phone_number']=$followUpData['phone_number'];
					$populateData['password']=$followUpData['password'];
					$populateData['session']=$followUpData['session'];
					$populateData['applicant_name']=$followUpData['applicant_name'];
					$populateData['email_id']=$followUpData['email_id'];
					$populateData['phone_number']=$followUpData['phone_number'];
					$populateData['application_no']=$followUpData['application_no'];
                  ...
                  //  $result = $academic_model->getSessionByCourseId($followUpData['degree']);
                   $this->view->session_id = $result[0]['session_id'];
					...
					$applicantUpdateData = array(
                               ...
                               'acad_year_id' =>$data['academic_year_list'],
                              
                            );	
							
						  $updateData=  $entrance_model->update($applicantUpdateData,array('application_no=?' => $followUpData['application_no']));
                      ...
                      //  if($data['csrftoken']===$token ){
                        ...
                        $entrance_model->update(array('degree'=>$data['degree_id'],'department' => $data['course'],'course' => $data['course'],'session'=>$data['session']),array('application_no=?' => $followUpData['application_no']));
                ...
                //}else{
                //    $this->_refresh(3,$this->_base_url."multi-step-form/index/type/step1/",'Invalid Token ..');
                //}
                //}
            }
       }
