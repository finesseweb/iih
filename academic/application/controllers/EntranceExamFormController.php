        ...
        //echo '<pre>'; print_r($token);exit;
            ...
            if(!empty($data['csrftoken'])) {
                ...
                if (empty($check_login)) {
                    ...
                    $applicatonNumber = trim($data['applicant_no']);
                    ...
                    {
                        $this->_redirectJs("multi-step-form/index/type/step1/a_id/" . $loginData['application_no']);
                    }
        ...
        if ($this->getRequest()->getPost()) {
            ...
            if(!empty($data['csrftoken'])) {
                ...
                if ($insert_id) {
                        ...
                        $this->_redirect("entrance-exam-form/welcome-page/a_id/" . $insert_id);
   ...
   // }
    ...
    public function welcomePageAction() {
        ...
        if (!empty($application_id)) {
            $applicantLoginDetails = $entrance_model->getRecordById($application_id);
            //$paymentData = $paymentData->getsavedData($application_no);
            //echo"<pre>";print_r($applicantLoginDetails); exit();
...
public function ajaxLoginAction() {
	 ...
	 $this->view->action_name = 'application';
       ...
       $this->accessConfig->setAccess('SA_ACAD_STUD');
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            //$phone = $this->_getParam("phone"); // Recipient mobile number
            $application_no = $this->_getParam("app_id");
           ...
           //echo '<pre>'; print_r($application_no);exit;
			$loginData = array(
                ...
                if (empty($check_login)) {
                    ...
                    $userdata['sess_vlue'] = $_COOKIE['PHPSESSID'];
                    //echo "<pre>";print_r($userdata);exit;
                    $userin= $user_log->insert($userdata);
                   
                    unset($_SESSION["token"]);
                  // $this->_redirect("entrance-exam-form");
				   if($userin) {
					  echo  md5($loginData['application_no']);
				   }
				   else {
