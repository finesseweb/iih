...
//        }
   ...
   //     
      ...
      public function noncolreportAction(){
   //     
        $this->view->action_name = 'noncolreport';
        $this->view->sub_title_name = 'noncolreport';
       // $this->accessConfig->setAccess('SA_ACAD_FINAL_GRADE');
        $student_report_form = new Application_Form_StudentFailReport();
         $Elective_model = new Application_Model_ElectiveSelection();
       ...
       // echo "hii"; die;
        if ($this->getRequest()->isPost()) {
                ...
                foreach($data['academic'] as $student_id => $academic_id)
                {
                    $mainelective = array();
                          foreach($data['term'][$student_id] as $indx => $term_id)
                        {
                            $mainelective['student_id'] = $student_id;
                            $mainelective['academic_year_id'] = $academic_id;
                            $mainelective['term_id'] = $term_id;
                            $mainelective['status'] = 0;
                            
                           $insert_id =  $Elective_model->insert($mainelective);
                          $itemsTable = array();
                            foreach($data['course'][$student_id][$term_id] as $key => $course_id)
                            {
                                $itemsTable['elective_id'] = $insert_id;
                                $itemsTable['students_id'] = $student_id;
                                $itemsTable['terms'] = $term_id;
                                $itemsTable['electives'] = $course_id;
                                 $itemsTable['ge_id'] = $data['ge'][$student_id][$term_id][$key];
                                 $backSelection->insert($itemsTable);
                            }
                        }  
                }
            $this->_flashMessenger->addMessage('Student Details Updated Successfully');
                                $this->_redirect('report/noncolreport');
...
public function studentProficencyReportAction(){
    ...
    public function getAttendanceReportAction() {
     ...
     public function getStudentFailReportAction() {
      ...
      public function getStudentNoncolReportAction() {
        $this->_helper->layout->disableLayout();
        $backselection = new Application_Model_BackSelectionItems();
        $tabul = new Application_Model_Academic();
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            $session = $this->_getParam("session");
            $cmn_terms_arr = $this->_getParam("cmn_terms");
            $cmn_terms = "('".implode("','",$cmn_terms_arr)."')";
           $resultabul =  $tabul->getFailRecordAccordingTotabulations($session,$cmn_terms);
           $resulback =  $backselection->getbackdata($session,$cmn_terms);
           $result=$this->your_array_diff($resultabul,$resulback);
      //      echo '<pre>'; print_R($result);exit;
            $this->view->session = $session;
            $this->view->department = $department;
            $this->view->terms = implode("_",$cmn_terms_arr);
...
$this->view->session = $session;
    ...
    public function getStudentReportAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            /// raushan change issue date 06-07-2023 //
            $issue_date = $this->_getParam("issuedate");

            $filter_status = $this->_getParam("filter_status");

            $this->view->term_id = $term_id;
            $this->view->year_id = $year_id;
            $this->view->issueDate = $issue_date;
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            if ($academic_id) {
                ...
                if (!empty($getacademicrecord)) {
                   ...
                   // echo "<pre>";print_r($termall);exit;
                    $Studentreport_model = new Application_Model_StudentPortal();
                     $term_ids = array_column($allterm,'term_id');
                  $result = $Studentreport_model->getStudentResultnew($academic_id, "'".implode("','",$term_ids)."'",$termall);
                               
                                $academicdetails = $academic_model->getRecord($academic_id);
                               //  $result = $grade_allocation_model->getStudentMarksDetails($academic_id,$term_ids,false,false,0,'','');
                     //  echo "<pre>";print_r($result);exit;         
...
//            }
    ...
    public function secondyeargradesheetreportAction() {
       // echo "<pre>";print_r($this->_act);exit;
        ...
        $type = $this->_getParam("type");
        $issue_date = $this->_getParam("issuedt");
        $cmn_terms = $term_id;
        $session = 0;
       ...
       /// raushan change issue date 06-07-2023
        $this->view->issue_date = $issue_date;
...
//        $pge_height+=150;
        ...
        $this->view->passpercent = $passpercent;
          ...
          //echo '<pre>';print_r($max[$value['term_id']]);exit;    
        $this->view->termcount = $termcount ;
    ...
    public function backPaperReportAction() {
        ...
        if (empty($term_id)) {
            ...
            if ($result) {
                ...
                $course_ids = $this->mergData($student_course_marks[$term_id], array('course_id'), count($student_course_marks[$term_id]));
             //   echo "<pre>";print_r($student_course_marks);die;
                if (empty($course_ids)) {
                    $this->_message('She is not in Non Collegiate group..');
                }
...
//echo "<pre>";print_r($this->_month);exit;
 ...
 public function ajaxGetFeesRegistrationRecordAction() {
    ...
    public function passFailReportAction(){
        ...
        case "grade":
               ...
               $students_id = array_column($student_details,"student_id");
...
//echo "<pre>";print_r($grade_result);exit;
                //   $result1  = $grade_Model->getStudentFailDetailsAccudeleted($session,"'".implode("','",$department)."'","('t1','t2','t3','t4','t5','t6')","2022-05-05",implode("','",$students_id));
                             
                //                   if(!$result1){
                                        
                //                                                 $overallpendingdel  = $grade_Model->getStudentFailDetailsAlldeleted($session,"'".implode("','",$department)."'","('t1','t2','t3','t4','t5','t6')","2022-05-05",implode("','",$students_id));
                                                               
                                                                
                //                                                 $overallpendingundel = $grade_Model->getStudentFailDetailsAlludeleted($session,"'".implode("','",$department)."'","('t1','t2','t3','t4','t5','t6')","2022-05-05",implode("','",$students_id));
              ...
              // $grade_result  = $grade_Model->getCGPAForStudentddegreepassfail("",implode("','",$stu_ids),"'t1','t2','t3','t4','t5','t6'",$degree,$session,"t6","2022-05-05");
    ...
    public function ajaxUpdateFeeRecordsAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
         //   echo "<pre>";print_r("hii");exit;
            ...
            if ((count($savingdata['data']) > 0) && $this->_filterstatus) {
               
            $savingdata['data'] = $this->filterPosts($savingdata['data'], true, $this->_allowedChars);
      ...
      public function updatesemfeerecords($savingdata,$type,$fidindex,$sem) {
           ...
           foreach($savingdata as $key => $value){
             //  echo "<prE>"; print_r($value);exit;
                ...
                $data = $payrecord[0]; 
                // $data['batch'] = $result['short_code'];
                // $data['name'] = $result['stu_fname'];
                // $data['class_roll']= $result['roll_no'];
                // $data['exam_id'] =  $result['exam_roll'];
                // $data['department'] = $result['dept_id'];
                // $data['phone'] = $result['stu_mobileno'];
                // $data['email'] =$result['stu_email_id'];
                //  $data['stu_id'] =$result['stu_id'];
                // $data['semester'] = 't3';
                // $data['fee'] = $value[5];
