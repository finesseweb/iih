        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            if ($col == 1) {
                ...
                if ($course_id != 0) {
                    ...
                    $result = $backStudentElective->getelectivestudentDetails1($academic_year_id, $term_id, $course['course_id'], 0, $type, $month);
                }
            } else {
    ...
    public function secondyeargradesheetreportAction() {
        ...
        if (empty($term_id)) {
          ...
          $course_credit_info[$value['term_id']]['sgpa_span'] = count($core_course_master->getCoreCousecreditCountno($academic_id, $value['term_id'], $course_ids));
              ...
              //   echo "<pre>"; print_r( $prom_text[$value['term_id']] );exit;
        }
...
//        $pge_height+=150;
        ...
        // print_r(array('tabl_id'=>$tbl_id,'term_id'=>$dateterm_id));EXIT;
        if ($publishDate) {
    ...
    public function backPaperReportAction() {
        ...
        if (empty($term_id)) {
            ...
            if ($result) {
                ...
                $course_credit_info[$value['term_id']]['sgpa_span'] = count($core_course_master->getCoreCousecreditCountno($academic_id, $term_id, $course_ids));
                //  echo $course_credit_info[$value['term_id']]['sgpa_span']; die;
...
//echo "<pre>"; print_r($result['session_id']);exit;
    ...
    public function ajaxGetNonCollegiateFormRecordAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $result = $backitem->getStudentForNonColRecords($academic_id, $sem, $exam_date);
            //echo '<PRE>';            print_r($result);DIE;
            $paginator_data = array(
                'result' => $result,
            );
