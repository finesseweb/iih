...
//                        $academic_id = $this->getRequest()->getPost('academic_id');
    ...
    public function ajaxGetPublishDateAction(){
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            foreach ($result as $k => $val) {
                echo '<option value="' . Date('Y-m-d',strtotime($val['result_publish_date']))."/".$val['id'] . '" >' . Date('d/m/Y',strtotime($val['result_publish_date'])) . '</option>';
            }die;
        }
    }
    ...
    public function ajaxGetGradeDetailsTrAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $batch_code = $academic_details->getRecord($academic_year_id)['batch_code'];
            $degree_id = $department->getRecord($dept_id);
          
            $dept_name = $degree_id['department'];
            $dept_des = $degree_id['description'];
            ...
            $resultStudents = $Items->getStudentMarksDetails($academic_year_id,array($term_id),$pay,$attendance,$tabTermId,$result[0]['cmn_terms']);
            ...
            $this->view->dept_desc = $dept_des;
            $this->view->batch_desc = $batch_code;
...
//        }
       ...
       public function ajaxGetGradeDetailsTrAllAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $dept_code = $degree_id['department'];
            $degree_id = $degree_id['degree_id'];
            if($degree_id)
              $passpercent = $this->getPasspercent($degree_id,$session);
                $resultTerm = $term_model->getRecordByAcademicId($academic_year_id);
                $term_ids = array_column($resultTerm,'term_id');
                $result = $Items->getStudentMarksDetails($academic_year_id,$term_ids,false,false,0,'',$prev);
                if(!$result)
                $result = $Items->getStudentMarksDetails($academic_year_id,$term_ids,false,false,0,'','');
           // $id  = $tr_report->getRecordsAcademicTerm($academic_year_id,$term_id);
    ...
    public function ajaxGetGradeDetailsTrBackAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()){
            ...
            $term_id = $this->_getParam("term_id");
             ...
             $pubmonth = $this->_getParam("pubmonth");
            $term_model = new Application_Model_TermMaster();
            $tr_report = new Application_Model_TabulationReport();
            $academic_details = new Application_Model_Academic();
            $department = new Application_Model_Department();
        ...
        //   $dept_arr = $GradeAllocation_model->getGroupebyCourseid($session_id,$course_id,'R');
            ...
            if ($grade_allocate_id) {
               ...
               foreach ($academic_id_Arr as $key => $value) {
                    if ($value) {
                        if (!$ct_id) {
                            $newData[$value] = $StudentPortal_model->getNotAddedStudents($value,$term_id,$course_id,$ct_id,100,0,$attend,$pay,$termidarr[$key]);
                        } else {
                            if($course_id)
                            $raw_data[$value] = $course_learning->getNotAddedgeStudentInForMarks($value, $course_id, $termidarr[$key], 'electives','',$ct_id,$attend,$pay);
                            $newData[$value] = $raw_data[$value];
                        //    echo "<pre>";print_r($newData[$value]);
                        }
                        foreach ($newData[$value] as $Studkey => $newVal) {
                            $newVal['academic_id'] = $value;
                            $newVal['term_id'] = $termidarr[$key];
    ...
    public function ajaxGetBaseOfCourseAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            foreach ($academic_id_Arr as $key => $value) {
                ...
                // echo '<pre>'; print_r( $result[$key]['course']);exit;
...
//=================[END]====================================================================//
    ...
    public function ajaxGetCoursesEditNew1Action() {
        $term_model = new Application_Model_TermMaster();
        $academic_model = new Application_Model_Academic();
        $grade_allocation_model = new Application_Model_GradeAllocation();
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $grade_allocation_id = $this->_getParam("grade_allocate_id");
               ...
               $grade_model =  $grade_allocation_model->getRecord($grade_allocation_id);
               $course_id = $grade_model['course_id'];
               $course_Detail = $course->getRecord($course_id);
            // $yearArr = $term_model->getRecordByCmnTerms($term_id, $session);
...
//            $academic_id_Arr = array_values(array_unique($academicYrId));
            ...
            //             if (array_key_exists($credit_val, $result[$key]['course']) && $credit_val!= $course_id) {
            //                 $main_res[$credit_val] = $result[$key]['course'][$credit_val];
            //             }
            //         }
            //     }
            ...
            echo '<select type="text" name="course_id" id="course_id" class="form-control chosen-select" required>';
            echo '<option value="">Select</option>';
            echo "<option value='".$course_id."' selected>".$course_Detail['course_name']."</option>";
...
/// ramesh  //	
    ...
    public function ajaxGetPrevDateAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
         ...
         $GradeAllocationItems_model = new Application_Model_GradeAllocation();
            $term_id = $this->_getParam("term_id");
            $islast = $this->_getParam("islast");
            $termno = $this->_getParam("termno");
            $lastterm = "t".$termno;
            $cmn_term = $this->_getParam("cmn_term");
            $resultacc=array();
            $term_ids = false;
            if($islast && $lastterm==$cmn_term){
               $termmodel = new Application_Model_TermMaster();
               $termdetails = $termmodel->getTermOnAcademicId($academic_year_id);
                $term_ids = array_column($termdetails,"term_id");
                $resultacc =  $GradeAllocationItems_model->getDistinctmonthofaccugrdes($academic_year_id);
                $date1 = array_column($resultacc,'vald'); 
            }
         ...
         $date2 = array_column($result,'vald');
             echo '<option value="">choose dates</option>';
              ...
              if($islast && $lastterm==$cmn_term){
                  $date = min($date1);
              $result =  $GradeAllocationItems_model->getDistinctmonthofdeletedgrdes($academic_year_id,$term_ids);
               $dates = array_merge($date1,$date2);
               foreach($resultacc as $k => $val)
              {
              ...
              echo '<option value="'.$val['vald'].'">'.$val['display'].'</option>';
               ...
               foreach($result as $k => $val)
              {
                if($val['vald'] > $date && !in_array($val['vald'],$dates))
              echo '<option value="'.$val['vald'].'">'.$val['display'].'</option>';
    ...
    public function ajaxGetPrevDate1Action() {
        $this->_helper->layout->disableLayout();
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
         $academic_year_id = $this->_getParam("academic_year_id");
               
            $GradeAllocationItems_model = new Application_Model_GradeAllocation();
            $examdates_model = new Application_Model_ExamDateModel();
            $termmodel = new Application_Model_TermMaster();
          $result =  $GradeAllocationItems_model->getDistinctmonthofaccugrdes($academic_year_id);
          $termdetails = $termmodel->getTermOnAcademicId($academic_year_id);
            $term_ids = array_column($termdetails,"term_id");
           $baseDate = $examdates_model->getRecordBForColor("t6",$academic_year_id)['result_publish_date'];
        //   echo "<prE>";print_r($baseDate);exit;
             echo '<option value="">choose dates</option>';
            // echo "<prE>";print_r($result);exit;
              foreach($result as $k => $val)
              {
                  
                if($baseDate==$val['vald']){
              echo '<option value="'.$val['vald'].'" style="background:#06693c;color:#fff">'.$val['display'].'</option>';
                }
                else
                {
                     echo '<option value="'.$val['vald'].'">'.$val['display'].'</option>';
              ...
              $dates = array_column($result,'vald');
                $date = min($dates);
                $result =  $GradeAllocationItems_model->getDistinctmonthofdeletedgrdes($academic_year_id,$term_ids);
                //=========================[02-08-2023]========================//
            //     foreach($result as $k => $val)
...
/// ramesh  //		
    ...
    public function tabulationRegisterAction() {
     ...
     if(!empty($course_id) && $_POST['studSaveId'][$stu_id]==0){
         $insertedRecord = $GradeAllocationReport_model->getSavedRecords($academic,$term,$course_id);
                   ...
                   $tab_insert_data = false;
                    //===============tabl_register=====//
                            $academic = $data['academic_year_id'];
                             ...
                             $publish_date = explode("/",$_POST['publish_date'])[0];
                             //echo $publish_date; die;
