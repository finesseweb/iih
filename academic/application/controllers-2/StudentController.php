...
class StudentController extends Zend_Controller_Action {
                    ...
                    if (!empty($data['csrftoken'])) {
                        if ($data['csrftoken'] === $token) {
                          ...
                          //  echo '<pre>';print_r($checkElective);exit;
                            ...
                            $elective_course_id = $course_credit_master->getErpCoreCouseDetailByTermAcademicCourseid($data['academic_id'], $data['term_id'], $ge);

                            $credit_result = $course_credit_master->getErpCoreCouseDetailByTermAcademicCourse($data['academic_id'], $data['term_id'], $elective_course_id);
 ...
 $checkcgpa = $StudentPortal_model->getStudenInfoByFormIdForPassoutCert($result['stu_id']);
                       //echo "<pre>";print_r($checkcgpa['grade_value']); die();
                           if(in_array('F',$checkcgpa['grade_value']) || in_array('NA',$checkcgpa['grade_value']) || in_array('Ab',$checkcgpa['grade_value']) || in_array('D',$checkcgpa['grade_value']))
                          {
                               $result['result_of_exam']= "Pending";
                          } else {
                   ...
                   // $result['name_of_university_exam']=$result['university'];
                   // commented 05-06-2023 raushan////
                   $result['name_of_university_exam']=$result['name_of_university_exam'];
                        ...
                        if ($_POST['csrftoken'] === $token) {
                           ...
                           // unset($data['selected_courses']);
                            ...
                            $data['university']= $data['university'];
                             $data['name_of_university_exam']= $data['name_of_university_exam'];
                           // echo"<pre>";print_r($result2);exit;
...
//                            }
                            ...
                            //$data['session'] = $session_info['session'];
                               $addon_course=$data['selected_courses'];
                             unset($data['selected_courses']);
...
//echo "<pre>";    print_r($data);exit;
                            // -------------[Auto Add into Elective Selection Items]-----------//
                            //===============[Get Term Name Start]=================================//
                            $data['selected_courses']=$addon_course;
                            $data['term_id'] = $term_master->getAcademicMinTerms($data['academic_id']);
...
//                
            ...
            case "add":
                if ($this->getRequest()->getPost()) {
                    ...
                    if (!empty($data['csrftoken'])) {
                        if ($data['csrftoken'] === $token) {
                            unset($data['csrftoken']);
                            foreach($data['term_id'] as $termindex => $term_id){
                            foreach ($data['fee_wave'] as $index => $dataFeetype) {
                                foreach ($data['selected_students'] as $arrindex => $stu_id) {
                                   ...
                                   // echo "<pre>";print_r($student_details);exit;
                                    ...
                                    $term_details = $termModel->getTermRecordsbycmn($student_details['academic_id'], $term_id);
                                    $dataStudent['fee_waiver'] = 1;
                                    $feewaiveData['stu_id'] = $stu_id;
                                    $feewaiveData['cmn_terms'] = $term_id;
                                    $feewaiveData['remarks'] = $data['remarks'];
                                    if ($dataFeetype == 1) {
                                        $examDate = $examDateModel->getDateInfoByAdmitCardStatus($student_details['academic_id'], $term_id,$dataFeetype);
                                        ...
                                        $feeExist = $examFeeNonColModel->getPaymentByTermIdcol($stu_id, $term_details['term_id']);
                                        if (!is_array($feeExist)) {
                                            $examFeeModel->insert($examFeeData);
                                            $FeeWaiverModel->insert($feewaiveData);
                                        }
                                    } else if ($dataFeetype == 2) {
                                        $examDate = $examDateModel->getDateInfoByAdmitCardStatus($student_details['academic_id'], $term_id);
                                        ...
                                        $fee = $feeitems->getFee($struct_id, $term_id);
                                        $accname = $fee;
                 ...
                 break;
                  case "edit":
                     $result =$FeeWaiverModel->getRecord($id);  
                     $result['fee_wave'] =$result['type']; 
                     $result['term_id'] =$result['cmn_terms'];
                     $result['remarks'] =$result['remarks'];
                     $res= $slection_form->populate($result);
                ...
                if ($this->getRequest()->getPost()) {
                                ...
                                foreach ($data['selected_students'] as $arrindex => $stu_id) {
                                   ...
                                   // echo "<pre>";print_r($student_details);exit;
                                    ...
                                    if ($dataFeetype == 1) {
                                        ...
                                        if (!is_array($feeExist)) {
                                            $examFeeModel->update($examFeeData,  array('u_id=?' => $student_details['stu_id'],'exam_month_id=?'=>$examDate['id']));
                                            $FeeWaiverModel->update($feewaiveData, array('id=?' => $id));
                                        }
                                    } else if ($dataFeetype == 2) {
                                        ...
                                        $feeExist = $examFeeNonColModel->getPaymentByTermId($stu_id, $term_details['term_id']);
                                        if (!is_array($feeExist)) {
                                            $examFeeNonColModel->update($examFeeData,array('u_id=?' => $student_details['stu_id'],'exam_month_id=?'=>$examDate['id']));
                                            $FeeWaiverModel->update($feewaiveData, array('id=?' => $id));
                                        }
                                    } else if ($dataFeetype == 3) {
                                        $struct_id = $feeStructure->getStructId($student_details['academic_id']);
                                        ...
                                        $feeExist = $tuitionfeesModel->getRecordbyFidTermid($stu_id, $feewaiveData['cmn_terms']);
                                        if (!is_array($feeExist)) {
                                            $tuitionfeesModel->update($insertdata,array('stu_id=?' => $student_details['stu_id'],'semester=?' =>$data['term_id']));
                                            $FeeWaiverModel->update($feewaiveData, array('id=?' => $id));
                                        }
                                    }
                                }
                 ...
                 case 'delete':
                if ($id) {
                    $FeeWaiverModel->trashItems($id);
                    $this->_flashMessenger->addMessage('Student Details Deleted Successfully');
                    $this->_redirect('student/selection');
                }
                break;
                default:
                     $messages = $this->_flashMessenger->getMessages();
    ...
    public function formStudentAction() {
        ...
        switch ($type) {
            ...
            case 'students':
                ...
                $result = $razor->getRecordByCouse($id,$acadid);
    ...
    //End 07 Oct
        public function ajaxGetAddonRecordByYearIdAction() {
        $this->_helper->layout->disableLayout();
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $this->view->year_id = $yearId;
            $this->view->d_id = $result['id'];
            $this->view->paginator = $this->_act->pagination($paginator_data);
        }
    }
    
    
       public function ajaxGetAddonRecordsAction() {
        $this->_helper->layout->disableLayout();
        ...
        //Modified by Kedar : 07 Oct 2020
    public function addonStudentAction() {

        $this->view->action_name = 'addonStudent';

        $this->view->sub_title_name = 'addonStudent';
        ...
        switch ($type) {
            ...
            case 'students':
                $did = $this->_getParam("d_id");
                $result = $razor->getAddonCourseDetails($id,$did);
                $page = $this->_getParam('page', 1);
                $paginator_data = array(
                    'page' => $page,
                    'result' => $result
  ...
  public function provisionalCertificateAction() {
        $this->view->action_name = 'studentinfo';
        $this->view->sub_title_name = 'student_detail';
        $this->accessConfig->setAccess('SA_ACAD_CHAR');
        $student_report_form = new Application_Form_StudentsAdmitcard();
    ...
    public function addonSelectionAction() {
        ...
        switch ($type) {
            ...
            case "getStudents":
                ...
                $this->view->session = $record['academic_year'];
                $this->view->paginator = $this->_act->pagination($paginator_data);
    ...
    public function ajaxGetStudentInfoForProvisionalAction() {
        $this->_helper->layout->disableLayout();
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            $erpStudent_model = new Application_Model_StudentPortal();
            $form_id = $this->_getParam("form_id");
    ...
    public function getStudentPassOutCardAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            if ( $getDepartMent['session'] == 1 || $getDepartMent['session'] == 2)
                $maxCountCheck = 6;
    ...
    //For Pass out student
    public function getStudentProvisionalCardAction() {
        $this->_helper->layout->disableLayout();
        $passOut_model = new Application_Model_PassOutModel();
        $streamModel = new Application_Model_DepartmentType();
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
    ...
    public function getStudentPassOutListAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            if ($academic_id) {
               ...
               // echo '<pre>';print_r($passOutNo);exit;
...
$applicantDetails['cgpa'] = $marksDetails['cgpa'];
    ...
    public function charprintAction() {
        ...
        $addoncourse =    new Application_Model_AddonCourseItemModel();
        $studentModel = new Application_Model_StudentPortal();
             $index = $this->_getParam("id");
             $dept = $this->_getParam("dept");
             $studetails = $_SESSION['itemresult'][$index];
            
            $grades =   $_SESSION['grades'];
            $exl = false;
            if(!$grades){ 
                $stuheader = $_SESSION['itemresult'][0];
           $grades = $refgradeaddons->getRefrencegradeAddons($studetails[count($studetails)-1]);
           $gradesvalues = $gradeskeys = array();
            foreach($grades as $value){
                $gradeskeys[] = array_keys($value);
                
                $gradeskeys[] = array_values($value);
            } 
            $grades =  array_values(array_map("unserialize", array_unique(array_map("serialize", $gradeskeys))));
            }
            else
            {
                $stuheader = $_SESSION['itemresult'][3];
            $exl = true;
            }
            ...
            if ($studetails){
                ...
                foreach($stuheader as $key => $value){
                    $keys = strpos($value,'.')?preg_replace('/\.+/', '', strtolower($value)):strtolower($value);
                  $result[strpos($keys,' ')?preg_replace('/\s+/', '_', $keys):$keys] = $studetails[$key];
                }
                $studetails=$studentModel->getStudenacademicDetailsnew($result['form_id']);
                
                $addondetails = $addoncourse->getRecordbyname($result['course_name'],$studetails['academic_year']);
          // echo "<pre>";print_r($addondetails);exit;
                $this->view->result = $result;
                $this->view->grades = $grades;
                $this->view->dept = $dept;
                $this->view->exl = $exl;
                $this->view->coordinator = $addondetails['filename'];
                $this->view->empl_id = $addondetails['empl_id'];
                $htmlcontent = $this->view->render('student/addoncertificate.phtml');
          //    echo $htmlcontent ; die;
                $this->_act->generateadmitcardPdfaddon($pdfheader, $pdffooter, $htmlcontent,  $result['form_id'], 'P', 150);
                die;
            }
    ...
    public function passoutprintAction() {
        ...
        if(!$applicantDetails['academic_id']){
        $this->_refresh(3,'/academic/student/pass-out-certificate/type/download','Certificate not Generated ..');
        }
...
$added_date['NonTblId'] = $examinationDates['result_publish_date'] > $NonCollegiateD ? "" :$added_date['NonTblId'];
    $added_date['RevalTblId'] = $examinationDates['result_publish_date'] > $revalD ? "" :$added_date['RevalTblId']; 
     ...
     ///echo '<pre>';print_r($cmn_terms);exit;
        ...
        /// echo '<pre>';print_r($getExamDate);exit;
 ...
 public function provisionalprintAction() {
        $this->_helper->layout->disableLayout();
        $this->_helper->layout->setLayout("applicationlayout");
        $stu_id = $this->_getParam("stu_id");
        $duplicate = $this->_getParam("type");
    ...
    $added_date['RevalTblId'] = $examinationDates['result_publish_date'] > $revalD ? "" :$added_date['RevalTblId']; 
     ...
     ///echo '<pre>';print_r($cmn_terms);exit;
      ...
      // echo '<pre>';print_r($applicantDetails);exit;
        ...
        $this->view->passOutDate = $passOutDate;
        $this->view->paginator = $applicantDetails;
        $htmlcontent = $this->view->render('student/provisionalprint.phtml');

        if ($check == 'admin' || $mode == 'view') {
            echo $htmlcontent;
            exit;
        }//======for PDF
        $this->_act->generateadmitcardPdfaddon($pdfheader, $pdffooter, $htmlcontent, $applicantDetails['stu_fname'] . $applicantDetails['stu_id'], 'P', 150);
    }
    ...
    public function studentDetailsAction() {
        ...
        switch ($type) {
            ...
            case "getStudents":
                ...
                $acadid = $academic_year->getRecordById($year_id);
                 //echo "<pre>";print_r($year_id);die;
                $result = $paymentModel->getPaidRecordByCourse($dept_id,$year_id);
            
                $result2 = $paymentModel->totalRecord($dept_id,$year_id);
                    
               // echo "<pre>";print_r($result);exit;
                $this->view->paginator = $result;
                $this->view->result2 = $result2;
                $this->view->year = $acadid['academic_year'];
                $page = $this->_getParam('page', 1);
...
//                echo "<b style='color:red'>Section already defined.</b>"; exit;
//            }
        $result = $student_model->getCompletedStudentsOnly($acad_id);
        $paginator_data = array(
            'page' => $page,
            'result' => $result
        );
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            if ($session_id) {
                ...
                foreach ($result as $key => $value) {
                    ...
                    $term_id = $term_master->getAcademicMinTerms($value['academic_id']);
                    $studentAddon = $addons->getStudentRecords($value['stu_id']);
                    $valuecourse = $valueCourseData->getRecordBystuid($value['stu_id']);
                    if(count($valuecourse)){
                    $result[$key]["Value-I"] = isset($valuecourse[0]['course_name'])?$valuecourse[0]['course_name']:" ";
                    $result[$key]["Value-II"] = isset($valuecourse[1]['course_name'])?$valuecourse[1]['course_name']:" ";
                    
                    $result[$key]["Semester-I"] = isset($valuecourse[0]['semester'])?$valuecourse[0]['semester']:" ";
                    $result[$key]["Semester-II"] = isset($valuecourse[1]['semester'])?$valuecourse[1]['semester']:" ";
                    }else{
                        
                         $result[$key]["Value-I"] = "NA";
                    $result[$key]["Value-II"] = "NA";
                    
                    $result[$key]["Semester-I"] = "NA";
                    $result[$key]["Semester-II"] = "NA";
                    }
                    
                    $addonsid = array_column($studentAddon,'addon_course_id');
                    if(count($addonsid)){
                    $courseAdon = $addoncourse->getRecordbyids($addonsid);
                    $result[$key]["Addons-I"] = isset($courseAdon[0]['name'])?$courseAdon[0]['name']:" ";
                    $result[$key]["Addons-II"] = isset($courseAdon[1]['name'])?$courseAdon[1]['name']:" ";
                    $result[$key]["Addonsingle"] = $result[$key]["Addons-I"].",".$result[$key]["Addons-II"] ;
                    }
                    else
                    {
                      $result[$key]["Addons-I"] = "NA";
                    $result[$key]["Addons-II"] ="NA";
                    $result[$key]["Addonsingle"] = "NA" ;   
                    }
    ...
    public function ajaxGetStudentBySessionForCreditsAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            if ($session_id) {
                $messages = $this->_flashMessenger->getMessages();
                $this->view->messages = $messages;
                $result = $student_model->getRecordsForEarnedcredit($session_id, $department_id);
                //echo '<pre>';print_r($result);exit;
                $this->view->paginator = $result;
...
//                    $result[$key]['core_id'] = $Student_course_details[0]['course_id'];
    ...
    public function formFeeCollectionAction() {
        ...
        switch ($type) {
            ...
            case "view":
                ...
                else if($_POST['examType'] === 'ET-RC')
                {
                    $formFeeDetails = $exammodel->getPaymentRecordbyfid(trim($_POST['stu_id']), $sem);
                }
                else {
            ...
            case "save":
               ...
               // echo '<pre>'; print_r($_POST); exit;
                $data = array(
                    ...
                    'reg_no' => $_POST['reg_no'],
                    'college_name' => "Patna Women's college",
                    'created_date' => $_POST['submit_date'],
                    'status' => 0,
                    'payment_status' => 1,
                    'u_id' => $_POST['stu_id'],
                    'payment_activation' => 1,
                ...
                //echo '<pre>'; print_r($data); exit;
                if ($_POST['examType'] === 'ET-C') {
                   $data['exam_month_id'] =  $_POST['exam_month_id'];
                    $insertId = $semFeeModel->insert($data);
                }
                else if ($_POST['examType'] === 'ET-RC') {
                    //echo 'test cooll';die;
                    $insertId = $exammodel->insert($data);
                }
                else {
                    //echo 'test nc';die;
    ...
    public function ajaxDeleteSemFeeDataAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            //print_r($short_code); die;
            $semFeeModel = new Application_Model_Seminarassociation();
            $updateData = array(
                'status' => 0,
                'f_code' => 0,
                'prod'=>""
    ...
    public function ajaxDeleteTcFeeDataAction() {
        $this->_helper->layout->disableLayout();
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            $delete_id = $this->_getParam("delete_id");
            //print_r($short_code); die;
            $semFeeModel = new  Application_Model_TcFeeCollection();
            $updateData = array(
                'status' => 0,
                'f_code' => 0,
                'prod'=>""
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
           ...
           $nonCollsemFeePayModel = new Application_Model_NonColpayment();
            ...
            if ($_POST['examType'] === 'ET-C') {
                ...
                //     'payment_status' => 0
                // );
                $updateId = $formFeeeeModel->delete(array('id=?' => $delete_id));
            }
             else if ($_POST['examType'] === 'ET-RC') {
                 $exammodel = new Application_Model_RegistrationPayment();
                // $updateData = array(
                ...
                //     'payment_status' => 0
                // );
                $updateId = $exammodel->delete(array('id=?' => $delete_id));
                $nonCollsemFeePayModel->delete(array('payment_id=?'=>$delete_id,'type=?'=>3));
            }else {
...
//Admission card from participant enrollment
    ...
    public function feesUpdationAction() {
                ...
                if ($this->getRequest()->getPost()) {
                   ...
                   if(!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $csvMimes)){
                    ...
                    if(!empty($name)){
                      ...
                      if(file_exists($dirPath.$name1))
                       unlink($dirPath.$name1);
        ...
        public function addonUpdationAction() {
        $this->view->action_name = 'formFee';
        $this->view->sub_title_name = 'formFee';
      //  $this->accessConfig->setAccess('');
        $this->accessConfig->setAccess('SA_ACAD_FORMFEECOLL');
        $student_report_form = new Application_Form_StudentsFees();
        //$academic_id = $this->_getParam("id");
        $type = $this->_getParam("type");
    ...
    public function ajaxGetStuDetailsAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            if ($reg_id) {
                ...
                $exam = $student_model->getExamRollByAcademic($academic_id);
                if($exam['exam_roll'])
                $result['exam_roll'] = $exam['exam_roll'];
              // echo '<pre>';print_r($result);exit;
    ...
    public function ajaxGetSelectionAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            if ($year_id) {
                $messages = $this->_flashMessenger->getMessages();
                $this->view->messages = $messages;
                $result = $FeeWaiverModel->getRecordsByAcid($year_id);
            //  echo '<pre>';print_r($result);exit;
        ...
        $studetails = new Application_Model_StudentPortal();
        $semFeeModel = new Application_Model_FeesCollection();
        $semLateModel = new Application_Model_Latefine();
        ...
        switch ($type) {
            ...
            case "view":
                ...
                $semFeeDetails = $semLateModel->getPayRecords(trim($_POST['stu_id']), $sem);
                //echo '<pre>'; print_r($semFeeDetails); exit;
                $studDetails = $studetails->getStudenInfoByStuIdForSemFee(trim($_POST['stu_id']));
            ...
            case "save":
               ...
               // echo '<pre>'; print_r($data); exit;
                $insertId = $semLateModel->insert($data);
                $this->_flashMessenger->addMessage('Semester fees succesfully submited.');
                $this->_redirect('student/sem-late-fine-collection');
     ...
     public function tcFeeCollectionAction() {
        $this->view->action_name = 'SEMFEECOLL';
        $this->view->sub_title_name = 'SEMFEECOLL';
        $this->accessConfig->setAccess('SA_ACAD_SEMFEECOLL');
        $student_report_form = new Application_Form_StudentsAdmitcard();
        ...
        $studetails = new Application_Model_StudentPortal();
        $semFeeModel = new Application_Model_FeesCollection();
        $semLateModel = new Application_Model_TcFeeCollection();
        ...
        switch ($type) {
            ...
            case "view":
                ...
                $semFeeDetails = $semLateModel->getStudentFeeTcRecordsstuid(trim($_POST['stu_id']), $sem);
             //   echo '<pre>'; print_r($semFeeDetails); exit;
                $studDetails = $studetails->getStudenInfoByStuIdForSemFee(trim($_POST['stu_id']));
            ...
            case "save":
                ...
                $submitDate = $splitdate[2] . "-" . $splitdate[1] . "-" . $splitdate[0];
             //   echo '<pre>'; print_r($_POST); exit;
             /// commented 50-06-2023 raushan   $semLateModel ///
             $get_mig_no= $semLateModel->getMigNo();
                ...
                $insertId = $semLateModel->insert($data);
                $this->_flashMessenger->addMessage('Semester fees succesfully submited.');
                $this->_redirect('student/tc-fee-collection');
...
//         $this->view->sub_title_name = 'student_detail';
//         $this->accessConfig->setAccess('SA_ACAD_CHAR');
//         $student_report_form = new Application_Form_AddonRegister();
//         $passOutModel = new Application_Model_PassOutModel();
//         //$academic_id = $this->_getParam("id");
//         $this->view->form = $student_report_form;
//         $type = $this->_getParam("type");
