...
class AddonGradeAllocationController extends Zend_Controller_Action {
    ...
    public function indexAction() {
        $this->view->action_name = 'addongradeallocation';
        $this->view->sub_title_name = 'addongradeallocation';
        $this->accessConfig->setAccess('SA_ACAD_GRADE_ALLOCATION');
        $GradeAllocation_model = new Application_Model_AddonGradeAllocation();
        $GradeAllocation_form = new Application_Form_AddonAllocation();
        $batchAttendance_form = new Application_Form_BatchAttendance();
        $GradeAllocatonItems_model = new Application_Model_AddonGradeAllocationItems();
                     ...
                     // echo "<pre>";print_r($grade);exit;
                        foreach (array_filter($grade['student_id']) as $k => $student_id) {
                           ...
                           // unset($data['term_id']);
                            ...
                            $studentNum = $student_info->getstudents($data['academic_year']);
                            ...
                            if (count($studentNum) > 0) {
                                ...
                                //$data['academic_id']=$studentNum['academic_id'];
...
//                                    }
                                    $data['flag'] = 'R';
                                $grade_id = $GradeAllocation_model->isGradeAllocated1($data['academic_year'], $data['employee_id'],$data['employee_id'], $data['term_id'], $data['course_id']);
                            ...
                            //==========================================[END]================================================//
                                $grade_data[$student_id]['grade_allocation_id'] = $last_insert_id;
                                $grade_data[$student_id]['student_id'] = $student_id;
                                $grade_data[$student_id]['grade_value'] = implode(",", $grade['grade_value_' . $student_id . '']);
                                $grade_data[$student_id]['number_value'] = implode(",", $grade['number_value_' . $student_id . '']);
                                $grade_data[$student_id]['obtained_marks'] =  array_sum($grade['number_value_' . $student_id . '']);
                                $grade_data[$student_id]['total_marks'] =  array_sum($grade['weightage']);
                                $grade_data[$student_id]['percent'] = $grade_data[$student_id]['total_marks']>0? number_format(($grade_data[$student_id]['obtained_marks']/$grade_data[$student_id]['total_marks'])*100,2):0;
                                $grade_data[$student_id]['component_id'] = implode(",", $grade['component_id']);
                                
                            } 
                            if(is_array($grade_data))
                            $GradeAllocatonItems_model->saveRows($grade_data);
                            unset($_SESSION["token"]);
                            $_SESSION['message_class'] = 'alert-success';
                            $this->_flashMessenger->addMessage('Grade Allocation Successfully added');
                           $this->_redirect('addon-grade-allocation/index');
                              }else {
            ...
            case 'edit':
                ...
                $result2 = $GradeAllocation_model->getGrouped($result1['academic_year'], $result1['employee_id'], $result1['course_id']);
                $GradeAllocation_form->populate($result1);
               // print_r($result2);die();
                $this->view->grade_allocate_id = $result2[0]['grade_arr'];
                 //Checking if it approved by D
                $GradeAllocationReportItems_model = new Application_Model_AddonGradeAllocationReportItems();

                $isExist1 = $GradeAllocationReportItems_model->isGradeReportPublished($result1['academic_year'], $result1['course_id']);
            ...
            if($grade['academic_id_' . $student_id . '_'.$grade_id][0]){
                ...
                $data['academic_id'] = $academic_id;
                 ...
                 $log_col_val = array(
                  ...
                  //  echo "<pre>";print_r($log_col_val);exit; 
...
//                        $academic_id = $this->getRequest()->getPost('academic_id');
                                    ...
                                    {
                                        $insert_grade_data[$student_id]['grade_allocation_id'] = $grade_data['grade_allocation_id'];
                                        $insert_grade_data[$student_id]['student_id'] =$grade_data['student_id'];
                                        $insert_grade_data[$student_id]['grade_value'] =$grade_data['grade_value'];
                                        $insert_grade_data[$student_id]['number_value'] =$grade_data['number_value'];
                                        $insert_grade_data[$student_id]['obtained_marks'] =$grade_data['obtained_marks'];
                                        $insert_grade_data[$student_id]['total_marks'] =$grade_data['total_marks'];
                                        $insert_grade_data[$student_id]['percent'] =$grade_data['percent'];
                                        $insert_grade_data[$student_id]['component_id'] =$grade_data['component_id'];
                                    }
                                } else {
                                    continue;
                                }
                        ...
                        if(is_array($insert_grade_data))
                            $GradeAllocatonItems_model->saveRows($insert_grade_data);
...
//                        $academic_id = $this->getRequest()->getPost('academic_id');
    ...
    public function ajaxGetGradeDetailsTrAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $Items = new Application_Model_AddonGradeAllocationItems();
            $addom_model = new Application_Model_AddonCourseModel();
           ...
           /// $dept_name = $degree_id['description'];
            ...
            if($academic_year_id)
               $passpercent = $this->getPasspercent($academic_year_id,$addon_id);
                
                
           // $id  = $tr_report->getRecordsAcademicTerm($academic_year_id,$term_id);
            $result = $addom_model->getRecordByAcad($addon_id, $academic_year_id);
          
           // $cmnTermNo = !empty($result[0]['cmn_terms'])?explode('t',$result[0]['cmn_terms']):0;
           // $tabulationTermsNo = $cmnTermNo[1]>1?$cmnTermNo[1]-1:$cmnTermNo[1];
           // $tabulationOneTermBack = 't'.$tabulationTermsNo;
           // $tabTerms = $term_model->getTermId($academic_year_id,$tabulationOneTermBack);
           // $tabTermId = !empty($tabTerms['term_id'])?$tabTerms['term_id']:0;
           $resultStudents = $Items->getStudentMarksDetailsWithoutTab($academic_year_id,$addon_id);
            $this->view->passpercent = $passpercent;
          //  $this->view->id =$id;
           $this->view->result = $result;
            $this->view->acad = $academic_year_id;
          //  $this->view->term = $term_id;
          //  $this->view->cmnTerms = $result[0]['cmn_terms'];
            $this->view->dept_name = $dept_name;
            $this->view->studentMarks = $resultStudents;
            $this->view->pay =$pay;
            $this->view->archive =$archive;
            $this->view->prev =$prev;
            $this->view->attend =$attendance;
            $this->view->addon_id =$addon_id;
        }
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            if($degree_id)
              $passpercent = $this->getPasspercent($degree_id,$session);
                ...
                $term_ids = array_column($resultTerm,'term_id');
                $result = $Items->getStudentMarksDetails($academic_year_id,$term_ids);
...
//             $yearidarr = array_column($yearArr, 'year_id');
//             $termidarr = array_column($yearArr, 'term_id');
//             
//             $academic_id_Arr = array_column($yearArr, 'academic_year_id');
             $GradeAllocation_model = new Application_Model_AddonGradeAllocation();
             $EvaluationComponentsItems_model = new Application_Model_AddonEvaluationComponentsItems();
             ///$addonallotement = new Application_Model_AddonCourseAssignmentModel();
              $addonallotement = new Application_Model_AddonCourseList();
        //     if(!empty($honors_id))
        //   $dept_arr = $GradeAllocation_model->getGroupebyCourseid($session_id,$course_id,'R');
           
               
            if (!empty($session_id) && !empty($honors_id)) {
                
                $academic_res_on_dept = $academic_model->getAcademicOnDept($honors_id, $session_id,$degree_id,$dept_arr['department']);
            ...
            $weight= $EvaluationComponentsItems_model->getWeightage($addon_course_list,$employee_id, $department_id, $course_id);
     //print_r($weight); die();
            $ReferenceGradeMasterItems_model = new Application_Model_AddonReferenceGradeItems();
            $ref_grades = $ReferenceGradeMasterItems_model->getRecordsByAcademicId($academic_year,$addon_course_list);
            ...
            $range = $ReferenceGradeMasterItems_model->getRecordsByNumgrade($min_pass_percent,$academic_year,$addon_course_list);
...
//                }
                ...
                $newData= $addonallotement->getStudentByAddonID($addon_course_list);
                 $this->view->category_data = $newData;
...
;
    ...
    public function ajaxGetCourseCodeAction() {
        $course_details = new Application_Model_AddonCourseMasterModel();
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
    ...
    public function ajaxGetBaseOfCourseAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $main_res = array();
            foreach ($academic_id_Arr as $key => $value) {
                $result[$key]['course'] = $EmployeeAllotment_model->gradeAllocationCourses($academic_id_Arr[$key], $department_id, $employee_id, $termidarr[$key],$cc_dept,$paper_flg = 'R',$month="",$ge_id,$attend,$pay,$cmn_term);
...
/// ramesh  //		
    ...
    public function tabulationRegisterAction() {
        ...
        $this->view->action_name = 'addontabulation';
        $this->view->sub_title_name = 'addontabulation';
        $this->accessConfig->setAccess('SA_ACAD_TRC');
        $GradeAllocationReport_model = new Application_Model_AddonGradeAllocationReport();
        $GradeAllocationReportItems_model = new Application_Model_AddonGradeAllocationReportItems();
        $tabulationReport = new Application_Model_AddonTabulationReport();
        $ge_master = new Application_Model_Ge();
        $tabulationReportItem = new Application_Model_AddonTabulationReportItems();
        $ge_id = new Application_Model_AddonLearningModel();
        $GradeAllocationReport_form = new Application_Form_AddonRegister();
        ...
        if ($this->getRequest()->isPost()) {
             //if ($GradeAllocationReport_form->isValid($this->getRequest()->getPost())) {
                    ...
                    $tab_insert_data = false;
                    
                    //===============tabl_register=====//
                        $academic = $data['academic_year'];
                             $term = $data['term_id'];
                             $datatblReport['academic_id'] = 0;
                             $datatblReport['term_id'] = 0;
                             ...
                             $datatblReport['academic_year'] = $academic;
                             $datatblReport['addon_course_list'] = $data['addon_course_list'];
                             if($_POST['saved_id']==0){
                    ...
                    foreach($stu_ids as $key => $stu_id){
                             ...
                             $datatblReportItems['total_grade_point'] =  $_POST['tot_grade_point'][$key];
                                ...
                                if($_POST['studSaveId'][$stu_id]==0){
                                    $tab_insert_data[$stu_id]['tabl_id'] = $datatblReportItems['tabl_id'];
                                    $tab_insert_data[$stu_id]['student_id'] =$datatblReportItems['student_id'];
                                    $tab_insert_data[$stu_id]['sgpa'] =$datatblReportItems['sgpa'];
                                    $tab_insert_data[$stu_id]['fail_in_ct_ids'] =$datatblReportItems['fail_in_ct_ids'];
                                    $tab_insert_data[$stu_id]['course_id'] =$datatblReportItems['course_id'];
                                    $tab_insert_data[$stu_id]['grade_point'] =$datatblReportItems['grade_point'];
                                    $tab_insert_data[$stu_id]['promotion_text'] =$datatblReportItems['promotion_text'];
                                    $tab_insert_data[$stu_id]['final_remarks'] =$datatblReportItems['final_remarks'];
                                    $tab_insert_data[$stu_id]['total_credit_point'] =$datatblReportItems['total_credit_point'];
                                    $tab_insert_data[$stu_id]['total_grade_point'] =$datatblReportItems['total_grade_point'];
                              }
                              else
                              {
                    ...
                    if(is_array($tab_insert_data))
                         // print_r($tab_insert_data); die();
                     $tabulationReportItem->saveRows($tab_insert_data);
                    //===========================[END]=======================//
                  ...
                  //  $term = $data['term_id'];
                    $addon_id = $data['addon_course_list'];
                    $data1['academic_year'] = $academic;
                    $data1['addon_course_list'] = $addon_id;
                    $data1['term_id'] = 0;
                    $data1['added_by'] = $this->login_storage->id;
                    $data1['added_date'] = $this->cur_datetime;
                    $data1['flag'] = 'R';
                    $data1['added_by_ip_address'] = $_SERVER['REMOTE_ADDR'];
     if(!empty($course_id) && $_POST['studSaveId'][$stu_id]==0){
         $insertedRecord = $GradeAllocationReport_model->getSavedRecords($academic,$addon_id,$course_id);
         $last_insert_id = !empty($insertedRecord['grade_report_id'])?$insertedRecord['grade_report_id']:$GradeAllocationReport_model->insert($data1);
     }
      foreach($stu_ids as $key => $stu_id){
                    $grade = $_POST['grade'];
                    //print_r($grade['grades_'. $stu_id . $academic . '_' . $addon_id . '_' . $course_id]); die();
         if($_POST['studSaveId'][$stu_id]==0){
                for ($k = 0; $k < count($_POST['grade']['grades_'. $stu_id . $academic . '_' . $addon_id . '_' . $course_id]); $k++) {
                    $item_data[$stu_id]['report_id'] = $last_insert_id;
                    $item_data[$stu_id]['term_id'] = $term;
                    $item_data[$stu_id]['course_id'] = $course_id;
                    $item_data[$stu_id]['tabl_id'] = $last_insert_id_tabl;
                   // $ge = $ge_id->getCoreCouseDetailByTermAcademicCourse($academic, $item_data[$stu_id]['term_id'], $item_data[$stu_id]['course_id']);
                    $item_data[$stu_id]['addon_course_list'] = $addon_id;
                    $item_data[$stu_id]['student_id'] = $stu_id;
                    $item_data[$stu_id]['sort'] = '';
                    $item_data[$stu_id]['ge_id'] = $ge['ge_id'] ? $ge['ge_id'] : 0;
                    if($item_data['ge_id'] != 0)
                    $name = $ge_master->getRecord($item_data[$stu_id])['general_elective_name'];
                    else
                        $name = '';
                    preg_match_all('/\bAECC/', $name, $output_array);
                    if(!empty($output_array[0]))
                     $item_data[$stu_id]['sort'] = 3;
                    preg_match_all('/\bSEC/', $name, $output_array);
                    if(!empty($output_array[0]))
                        $item_data[$stu_id]['sort'] = 4;
                    preg_match_all('/\bDSE/', $name, $output_array);
                    if(!empty($output_array[0]))
                        $item_data[$stu_id]['sort'] = 5;
                    else if($item_data['ge_id'] == 0){
                        $item_data[$stu_id]['sort'] = 1;
                    }
                else if(empty($item_data[$stu_id]['sort'])){
                    $item_data[$stu_id]['sort'] = 2;
                 }
                    
                    
                    $item_data[$stu_id]['component_grades'] = $grade['grades_' . $stu_id . $academic . '_' . $addon_id . '_' . $course_id][$k];
                    $item_data[$stu_id]['component_weightages'] = $grade['weightages_' . $stu_id . $academic . '_' . $addon_id . '_' . $course_id][$k];
                    $item_data[$stu_id]['grade_point'] = $grade['grade_point_' . $stu_id . $academic . '_' . $addon_id . '_' . $course_id][$k];
                ...
                $GradeAllocationReportItems_model->saveRows($item_data);
             }
            $this->_flashMessenger->addMessage('Grade Report Successfully added');
             $_SESSION['message_class'] = 'alert-success';
            $messages = $this->_flashMessenger->getMessages();
            $this->view->messages = $messages;
            $this->_redirect('addon-grade-allocation/tabulation-register');
             //}
                ...
                if(!empty($_POST['c_id'][$key])){
                    ...
                    //===================[END Updating]=================//
                              ...
                              if($_POST['studSaveId'][$stu_id]==0){
                                    ...
                                    $tab_insert_data[$stu_id]['total_credit_point'] =$datatblReportItems['total_credit_point'];
                                    $tab_insert_data[$stu_id]['total_grade_point'] =$datatblReportItems['total_grade_point'];
                                        $tab_insert_data[$stu_id]['publish_date'] =$datatblReportItems['publish_date'];
                              }
                              else
                              {
                                 $tabulationReportItem->update($datatblReportItems,array('tabl_id=?'=>$last_insert_id_tabl,'student_id=?'=>$datatblReportItems['student_id'])); 
      ...
      foreach($stu_ids as $key => $stu_id){
                ...
                for ($k = 0; $k < count($_POST['grade']['grades_'. $stu_id . $academic . '_' . $term . '_' . $course_id]); $k++) {
                    ...
                    $item_data[$stu_id]['ge_id'] = $ge['ge_id'] ? $ge['ge_id'] : 0;
                      ...
                      if($item_data[$stu_id]['ge_id'] != 0)
                    $name = $ge_master->getRecord($item_data[$stu_id]['ge_id'])['general_elective_name'];
                    else
                        $name = '';
                    preg_match_all('/\bAECC/', $name, $output_array);
                    if(!empty($output_array[0]))
                    ...
                    $item_data[$stu_id]['component_grades'] = $grade['grades_' . $stu_id . $academic . '_' . $term . '_' . $course_id][$k];
                    $item_data[$stu_id]['component_weightages'] = $grade['weightages_' . $stu_id . $academic . '_' . $term . '_' . $course_id][$k];
                    $item_data[$stu_id]['grade_point'] = $grade['grade_point_' . $stu_id . $academic . '_' . $term . '_' . $course_id][$k];
    ...
    public function getPasspercent($academic_year_id='',$addon_id){
            try{  
                if(!empty($academic_year_id)){
            $ReferenceGradeMasterItems_model = new Application_Model_AddonReferenceGradeItems();
            $ref_grades = $ReferenceGradeMasterItems_model->getRecordsByAcademicId($academic_year_id,$addon_id);
            ...
            $range = $ReferenceGradeMasterItems_model->getRecordsByNumgrade($min_pass_percent,$academic_year_id,$addon_id);
