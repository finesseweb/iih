        ...
        switch ($type) {   
            ...
            //$assignment_model->populate($result);
                ...
                if ($this->getRequest()->isGet()) {
                    ...
                    $sem = trim($_GET['sem']);
                      ...
                      if(strlen($sem)>1 && $sem != 't1'){
                          ...
                          //   print_r($prevExamFormDetails); die(); 
                           if($prevExamFormDetails['payment_status']==0 || $prevExamFormDetails['payment_activation']==0){
                                 $this->_refresh(3,'/academic/application/examform/type/otpverify','You Are Not Allowed . Please Consult your college Department..!');
                           }
                        }
                     $result = $assignment_model->getStudenFullInfo($stu_id);
                       if(empty($result)){
                    $this->_refresh(5,'/academic/application/examform/type/otpverify?v2','You are Not Allowed Please Consult college Department..!');
                     }
                      
                    $details_stu = $assignment_model->getPermotedStudenacademicDetails($result['stu_id'],$result['academic_id'],$sem);
                     
             if(empty($details_stu)){
                    $this->_refresh(5,'/academic/application/examform/type/otpverify?v2','You are Not Promoted Please Consult college Department..!');
            }
                            $dept_id = $dept_model->getRecordbyAcademic($result['academic_id']);
                            $cf = new Application_Model_Coursefee();
                            if($dept_id){
                             $fe_details = $cf->getRecordByDepartmentandSession($dept_id['department_id'],$result['session_id'],$sem,1);
                             if(!$fe_details){
                                 $this->_refresh(5,'/academic/application/examform/type/otpverify?v2','Not Allowed For this session..!');
                             }
                            }
             ...
             // $payinform = $examsubmit->isPaymentExist($stu_id,$sem);
                    if($payinform['payment_status']=='1' && $payinform['payment_activation']=='1'){
                       // $this->_refresh(5,'/academic/application/examform/type/otpverify','NOTE:-  Admit card will be provided by the college on 02-01-2023 (After the General Assembly)');
                      //print_r($payinform);
                       //die();
                ...
                if ($this->getRequest()->isPost()) {
                  if(isset($_POST)){
                     ...
                     $res = $otpmodel->getVerifyotp($uid,$otp,$semester);
                      ...
                      $this->_refresh(5,'/academic/application/examform/type/otpverify',"Invalid Form ID OR Pending Semester Fees OR You have Choosen Wrong Semester !");
                      }
            ...
            $sem_fees = new Application_Model_FeesCollection();
                        ...
                        //   echo "<pre>".print_r($res);exit;
                          // echo $acad_id; die;
         $examdate=  $fee_model->getEnddate($semester,$res['academic_id'],$res['session']);
        ...
        $paidfee = array();
                  ...
                  $due = $sem2-$totfee;
                   ...
                   // print_r($examdate);
                    ...
                    if(!empty($examdate && $res['session']==$examdate['session'])){
...
public function admitcardAction() {
   ...
   $this->_refresh(5,'/academic/application/examform/type/otpverify','NOTE:-  Admit card will be provided by the college  (After the General Assembly)');
     die();
             ...
             $this->view->form = $assignment_form;
                ...
                // echo "<pre>"; print_r($result);exit;
                  ...
                  $course_details_arr = $this->geStudentCourse($acad_term_arr['batch_id'],$acad_term_arr['term_id'],$result['student_id']);
                   ...
                   $this->view->course_val = $selectge;
                   $this->view->term_id = $acad_term_arr['term_id'];
                   $this->view->ge_id = $this->aeccConfig[0];
       ...
       public function ajaxGetAdmissionCardAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
             $paymentModel = new Application_Model_StudentPortal();
             $geinfo = new Application_Model_Ge();
           
              $stu_id = $this->_getParam("stu_id");
                    
                $result = $paymentModel->getAllDetailsStudents($stu_id);
    ...
    // die();
      ...
      //  echo "server issue";die;
       ...
       // $assignment_form = new Application_Form_AdmitCardView();
        ...
        $examchecker = new Application_Model_Examchecker();
                ...
                if ($this->getRequest()->isGet()) {
                 ...
                 // echo "<prE>";print_r($checkk);exit;
                    ...
                    $details_stu = $student_model->getStudenInfoByU($stu_id);
                     ...
                     $examcheckers=date('Y');
                     /// $examcheckers=$examchecker->getAcademicYear($checkk['0']['session'],date('Y'),$_GET['sem']);
                       $datecheckers=$examchecker->getRecordByIds($checkk['0']['session'],$_GET['sem'],$details_stu['stu_id']);
                     // echo "<pre>";print_r($details_stu); die();
                     if($examcheckers > $datecheckers['last_attempt_year']){  
                       $this->_refresh(5,'/academic/application/noncollegiate/type/otpverify','Please contact your examination department you have no attempt left');
                    }
                      else {   
                     if($_GET['sem']!='t1'){
                       $getpreviousrecor=  $examchecker->getPreviousrecord($_GET['sem'],$datecheckers['session'],$details_stu['academic_id'],$details_stu['student_id']);
                        if($getpreviousrecor ){
                       $this->_refresh(5,'/academic/application/noncollegiate/type/otpverify','Please contact your examination department you have not clear your previous exam');
                    }
                }
                   ...
                   // echo "<pre>"; print_r($acad_id);exit;
                    ...
                    if(!$exam_month)
                    $this->_refresh(5,'/academic/application/noncollegiate/type/otpverify','Please Contact Examination Department !');
                  ...
                  if(isset($_POST)){
                     ...
                     //$res = $otpmodel->getVerifyotpnon($uid,$otp);
                      ...
                      if(empty($res) || $res['academic_id']<=0){
                            ...
                            $this->_refresh(5,'/academic/application/noncollegiate/type/otpverify','Wrong Form Id or Semester');
...
public function admitcardnoncollegiateAction() {
              $this->_refresh(5,'/academic/application/noncollegiate/type/otpverify','NOTE:-  Admit card will be provided by the college   (After the General Assembly)');
    //die();
        $this->view->action_name = 'admitcard';
        $this->view->sub_title_name = 'admitcard';
        $this->accessConfig->setAccess('SA_ACAD_ADMIT_CARD');
    ...
    public function noncolreportcardAction() {
     ...
     public function studentTermReportAction($stu_id = '',$otp='') {
       ...
       if($student_details['academic_id']!=$acad){
             $this->_refresh(5,'/academic/application/marksheet','please Contact concerned Office for your Marksheet..!');
        }
        if(!$student_details ){
 ...
 public function getBackStudentMarks($academic_id, $tabulation_id, $stu_id, $term_id) {
        ...
        $getStudent_course_details = $GradeAllocationReport->getMarksDetailsWithCourse($tabulation_id, $stu_id, $term_id);
         $examDateModel = new Application_Model_ExamDateModel();
               ...
               $examinationDates = $examDateModel->getRecordByAcadIdstu($requiredData);
    ...
    //   echo "<pre>";print_r($formated_result);exit;
        ...
        //===================[GRADE SHEET NUMBER]===============================//
               ...
               // $gradesheet_number = $GradeSheet_model->getGradeSheetNumber($academic_id, 1, $stu_id,0);
                ...
                ///Raushan 12-07-2023  add serial no in scanner same as print out ///
               // $GradeSheet_model = new Application_Model_PassOutModel();
               // $gradesheet_number = $GradeSheet_model->getGradeSheetNumber($academic_id, $stu_id, 0);  
                //==========================changed 14-07-2023==================================//
                if ($year_id) {
          ...
          $transactionRequest  = new TransactionRequest();
           switch ($type) {   
               case "add":    
                   ...
                   // http_response_code(404); die;
                   $details_stu = $studetails->getStudenInfoByU(md5($_POST['registraion_num']));
                    $acad_id = $details_stu['academic_id'];
                 
               $term_ids = $term_obj->getTermRecordsByTerm1($acad_id,$_POST['term_id']);
          //  print_r($term_ids);
            //    die();
                       ...
                       'u_id' => $_POST['registraion_num'],
                        ...
                        'submit_date' => date('Y-m-d')
                             ...
                             $cf = new Application_Model_Coursefee();
                             $fe_details = $cf->getRecordByDepartmentandSession($dep_id,$ac_details['session'],$term_ids['cmn_terms'],1);
                      ...
                      date_default_timezone_set('Asia/Calcutta');
                        ...
                        $transactionRequest->setCustomerId($_POST['exmination_id']."|".$details['roll_no']."|".$term_ids['cmn_terms']);
   ...
   //        echo "Testing..."; die;
                      ...
                      if($data['academic_year_id']==0 || empty($data['academic_year_id'])){
                            ...
                            if(!$dept_id['department']){
                               ...
                               //  print_r($data);die();
                                $fee_pay = $fee_model->getColByDepartment1($dept_id['department'],$data['academic_year_id'],$data['term_id'],$data['session_id']);
                         //     print_r($fee_pay);die();
                     ...
                     $term_ids = $term_obj->getTermRecordsByTerm1($acad_id,$_POST['term_id']);
                       ...
                       'u_id' => $_POST['registraion_num'],
                        ...
                        'submit_date' => date('Y-m-d')
                             ...
                             $dep_id = $ac_details['department'];
                             $cf = new Application_Model_Coursefee();
                               $fe_details = $cf->getRecordByDepartmentandSession($dep_id,$ac_details['session'],$term_ids['cmn_terms'],2);
                      ...
                      if($data['academic_year_id']==0 || empty($data['academic_year_id'])){
                            ...
                            if(!$dept_id['department']){
                                $this->redirectExamform('Access denied by Administrator',$_GET['col']);
                                 ...
                                 $fee_pay = $fee_model->getNonColByDepartment($dept_id['department'],$data['academic_year_id'],$data['term_id'],$data['session_id']);
                      
                                 // echo "<pre>"; print_r($fee_pay);die();
                                  if($fee_pay['feeForm_start_date']<=date('Y-m-d') || $fee_pay['feeForm_extended_date'] <=date('Y-m-d')){
     ...
     public function registrationAction() {
      ...
      //  echo "server issue";die;
       ...
       // $assignment_form = new Application_Form_AdmitCardView();
        ...
        $student_model = new Application_Model_StudentPortal();
        $stu_obj = new Application_Model_StudentPortal();
        $examsubmit = new Application_Model_RegistrationPayment();
        $examchecker = new Application_Model_Examchecker();
            ...
            //$assignment_model->populate($result);
             ...
             $this->view->form = $assignment_form;
                ...
                if ($this->getRequest()->isGet()) {
                    ...
                    $checkk[0] = $details_stu;
                   $checkk[0]['terms'] = $term_ids['term_id'];
                   $checkk[0]['session'] =  $term_ids['session'];
                   $payinform = $examsubmit->checkPaymentRecord($stu_id,$sem,$exam_month['id']);
                 // print_r($exam_month['id']); die();  
         //  echo $stu_id;die;
        // echo "<pre>";print_r($payinform);exit;
                   if($payinform['payment_status']=='1' && $payinform['payment_activation']=='1'){
                       
                      $this->_redirect("application/regresponse?u_id=".$payinform['u_id']."_".$payinform['payment_id']."_".$payinform['pay_id']);
                   }
                   elseif($payinform['payment_activation']=='1'){
                      
                       $this->_redirect('application/regpayment/?fid='.$stu_id);
                   }
                   else
                   {
                   ...
                   $this->view->checkk =$checkk;
                    ...
                    $this->view->acd_id = $checkk[0]['academic_id'];
                    $this->view->st_id = $stu_details['student_id'];
                    $this->view->stu_dob = $stu_details['stu_dob'];
                    $this->view->registration_no = $stu_details['reg_no'];
                    $this->view->exam_roll = $stu_details['exam_roll'];
                    $this->view->father_fname = $stu_details['father_fname'];
                       $this->view->college = "Patna Women's College";
            ...
            case 'save':
                ...
                $examsubmit_model = new Application_Model_RegistrationPayment();
                $examform = new Application_Form_AdmitCardView();
                if ($this->getRequest()->isPost()) {
                //  print_r($_POST);die();
                    if (isset($_POST)) {
                        ...
                        $attendanceInfoData = array(
                           ...
                           'examination_id' => $_POST['exam_roll'], 
                           'college_name' => $_POST['college'],
                         'cc_paper_name' => $_POST['cc_paper_name'],
                           'ge_paper_name' => $_POST['ge_paper_name'],
                           'aecc_paper_name' => $_POST['aecc_paper_name'],
                           'created_date' => date('Y-m-d'),
                           'u_id' => $_POST['f_id'],
                           'email' => $_POST['email'],
                           'phone' => $_POST['phone'],
                         ...
                         $this->_flashMessenger->addMessage('Attendance Successfully added');
                        $this->_redirect('application/regpayment/?fid='.md5($_POST['f_id']));
                        //print_r($_POST);
                        //die();  
                    }
                    }
                break;
            case 'otpverify':
                  $semdata=  $fee_model->getRegRecord();
                 $this->view->showsem=$semdata; 
                //  echo "<pre>"; print_r($semdata);exit;
                if ($this->getRequest()->isPost()) {
                  if(isset($_POST)){
                     ...
                     //$res = $otpmodel->getVerifyotpnon($uid,$otp);
                      ...
                      if(empty($res) || $res['academic_id']<=0){
                            ...
                            $this->_refresh(5,'/academic/application/registration/type/otpverify','You have entered wrong form id.');
                    
                      }
                  
                      $examdate=  $fee_model->getregEnddate($semester,$res['academic_id']); 
                    //   print_r(empty($examdate));
                    //   die();
                      if(!empty($examdate)){
                        ...
                        if($examdate['feeForm_extended_date']!='0000-00-00'){
                          if(date('Y-m-d')>=$examdate['feeForm_start_date'] && date('Y-m-d')<=$examdate['feeForm_extended_date']) {  
                           ...
                           $this->_refresh(5,'/academic/application/registration/type/otpverify','Form filling not started for this Academic! OR End Date');
                      ...
                      $this->_redirect('application/registration/type/add?stu_id='.md5($_POST['stu_id']).'&sem='.$semester);
                    
                     }
                      else {    
                            $this->_refresh(5,'/academic/application/registration/type/otpverify','Invalid OTP OR You have Choosen Wrong Semester');
                      
                      }
                    }
                    else
                    {
                        $this->_refresh(5,'/academic/application/registration/type/otpverify','Form not started yet .');
                    }
            ...
            case 'delete':
                ...
                break;
            default:
             $this->_redirect('application/registration/type/otpverify');
                 if ($this->getRequest()->isPost()) {
                 if (isset($_POST)) {
                     $rand= rand(999,10000);
//                     $stunum = $assignment_model->getStudentM($_POST['stu_id']);
//                     $num =  $stunum['father_mobileno'];
     ...
     public function regpaymentAction() {
    ...
    //http_response_code(404); die;
        require_once APPLICATION_PATH . '/public/atompay/TransactionRequest.php';
         ...
         $pay_form = new Application_Form_PaymentForm();
          $this->view->form = $pay_form;
          $semattend = new Application_Model_BatachSemesterAttendance();
          $exammodel = new Application_Model_RegistrationPayment();
          $examfee = new Application_Model_NonColpayment();
          $studetails = new Application_Model_StudentPortal();
                     ...
                     $term_ids = $term_obj->getTermRecordsByTerm1($acad_id,$_POST['term_id']);
                       ...
                       'u_id' => $_POST['registraion_num'],
                        ...
                        'submit_date' => date('Y-m-d')
                             ...
                             $dep_id = $ac_details['department'];
                             $cf = new Application_Model_Coursefee();
                             $fe_details = $cf->getRecordByDepartmentandSession($dep_id,$ac_details['session'],'t1');
                             
                            $last_id =$_POST['last_id'];
                       
                            $last_insert_id = $examfee->insert($insertdata);
                           $mail_data =  $exammodel->getRecordbyfid3($_POST['registraion_num']);
                          // echo "<prE>";print_r($fe_details['product_id']);exit;
                      //$exammodel->update(array('payment_status'=> 1), array('u_id=?' => $sfid))  ;
                      $this->_flashMessenger->addMessage('Payment Successfully now You can Download your Admit Card');
                      date_default_timezone_set('Asia/Calcutta');
                        $datenow = date("d/m/Y h:m:s");
                        ...
                        $transactionRequest->setLogin(53243);
                        $transactionRequest->setPassword("Patna@1234");
                        $transactionRequest->setProductId(str_replace(" ","_",strtoupper(trim($fe_details['product_id']))));
                        $transactionRequest->setAmount($_POST['total_fee']);
                        $transactionRequest->setTransactionCurrency("INR");
                        $transactionRequest->setTransactionAmount($_POST['total_fee']);
                        $transactionRequest->setReturnUrl($this->_base_url  . 'application/regresponse/?u_id='.$_POST['registraion_num']."_$last_id"."_$last_insert_id");
                        $transactionRequest->setClientCode(53243);
                        $transactionRequest->setTransactionId($transactionId);
                        $transactionRequest->setTransactionDate($transactionDate);
                        $transactionRequest->setCustomerName($_POST['stu_name']);
                        $transactionRequest->setCustomerEmailId($mail_data['email']);
                        $transactionRequest->setCustomerMobile($mail_data['phone']);
                        $transactionRequest->setCustomerBillingAddress($_POST['registraion_num']."|".'Registration');
                        $transactionRequest->setCustomerId($_POST['exmination_id']."|".$details['roll_no']."|".$term_ids['cmn_terms']);
                        $transactionRequest->setCustomerAccount("639827");
                       $transactionRequest->setReqHashKey("b338c08a991313f12c");
         ...
         //  die();
             ...
             $this->view->termid = $data['term_id'];
                      ...
                      if($data['academic_year_id']==0 || empty($data['academic_year_id'])){
                                ...
                                $this->_refresh(3,'/academic/application/registration/type/otpverify','Your Form Id not present in academic!');
                        } else {
                            $dept_id = $academic_model->getRecord($data['academic_year_id']);
                            if(!$dept_id['department']){
                                $this->_refresh(3,'/academic/application/registration/type/otpverify','Access denied by Administrator');
                            }else{
                                
                                 $fee_pay = $fee_model->getregByDepartment($dept_id['department'],$data['academic_year_id'],$data['term_id'],$data['session_id']);
                      
                                  // print_r($fee_pay);die();
                                  if($fee_pay['feeForm_start_date']<=date('Y-m-d') || $fee_pay['feeForm_extended_date'] <=date('Y-m-d')){
                               
                                     if(!$fee_pay['examFee']){
                                $this->_refresh(3,'/academic/application/registration/type/otpverify','No Exam Fee Applicable');}
                                 else{
                        ...
                        if($difference->d > $perDayinc){
                           ...
                           // echo $addfineTimes; die();
                            $finefee=0;
                             ...
                             $this->view->per_day = $fee_pay['fineFee'];   
                            $this->view->examfeena = $multi; 
                            $this->view->fine_days = $difference->d;
                        }
                        else {
                             $this->view->examfeena = $fee_pay['fineFee']; 
                             $this->view->per_day = $fee_pay['fineFee'];
                             $multi=$fee_pay['fineFee']; 
                             $this->view->fine_days = 1;
                        }
...
public function regresponseAction(){
    require_once APPLICATION_PATH . '/public/atompay/TransactionResponse.php';
            $exammodel = new Application_Model_RegistrationPayment();
      ...
      ///  raushan commented 06-06-2023///      
        $transactionResponse = new TransactionResponse();
            $transactionResponse->setRespHashKey("8b013258b7a4b89428");
             $transactionResponse->setResponseEncypritonKey("19DE2650AF672D308C508346BDDE0272");
    $transactionResponse->setSalt("19DE2650AF672D308C508346BDDE0272");
    $arrayofdata = $transactionResponse->decryptResponseIntoArray($_POST['encdata']);
      $this->view->vaild_response = $transactionResponse->decryptResponseIntoArray($_POST['encdata']);
    $_POST = $arrayofdata;     
    ///  raushan commented 06-06-2023///  
            ...
            $updatedata = array(
                ...
                'type'=>3
                 ...
                 $result = $exammodel->getRecordbypaymentid($last_id);
                 $batch = $academic->getAcademicdepartment($result['academic_year_id']);
                 $result['department'] = $batch['department'];
           
                    if($_POST['f_code'] == 'Ok' || $result['f_code'] == 'Ok'){
                    if($_POST['f_code'] == 'Ok'){
                    ...
                    $this->view->response_data = $result;
             }
      ...
      public function responseNewAdmissionAction(){
          ...
          // comment below code 06-06-2023 raushan//
           //  $transactionResponse = new Application_Model_TransactionResponse();
          //  $transactionResponse = new Application_Model_TransactionResponse();
    ...
    $transactionResponse->setSalt("19DE2650AF672D308C508346BDDE0272");
    $arrayofdata = $transactionResponse->decryptResponseIntoArray($_POST['encdata']);
     $this->view->vaild_response = $transactionResponse->validateResponse($_POST['encdata']);
    $_POST = $arrayofdata;
             $getid = $_GET['cid'];
...
public function responseAction(){
    require_once APPLICATION_PATH . '/public/atompay/TransactionResponse.php';
        ...
        // raushan add below code 06-06-2023 ////
       $transactionResponse = new TransactionResponse();
        $transactionResponse->setRespHashKey("8b013258b7a4b89428");
               $transactionResponse->setResponseEncypritonKey("19DE2650AF672D308C508346BDDE0272");
    $transactionResponse->setSalt("19DE2650AF672D308C508346BDDE0272");
    ...
    public function ajaxGetBackCourseFeeAction() {
     ...
     public function ajaxGetStudentCourseAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
         ...
         // echo "<pre>"; print_r($course_details_arr);die;
            $core_course_name_arr = $this->mergData($course_details_arr, array('cc_name'), count($course_details_arr));
            $ge_arr = $this->mergData($course_details_arr, array('ge_id'), count($course_details_arr));
    ...
    //http_response_code(404); die;
          ...
          $transactionRequest  = new TransactionRequest();
           switch ($type) { 
               ...
               $payrecord =$sem_fees->getPayTotRecords(trim($_POST['stu_id']),$_POST['semester']);
                    ...
                    if(empty($acad_stu['academic_id']) && $_POST['semester'] == 't1'){
                  ...
                  if(empty($acad_stu['academic_id'])){
                        ...
                        if(empty($acad_stu['academic_id'])) {
                           $this->_refresh(3,'/academic/application/tuitionfees?v2','Please Contact Your Concern Office ..');
        ...
        $paidfee = array();
               ...
               if($_POST['semester'] != 't1'){
                  $details_stu1 = $studetails->getStudenacademicDetails(trim($_POST['stu_id']));
                    $details_stu = $studetails->getPermotedStudenacademicDetails(trim($_POST['stu_id']),$details_stu1['academic_id'],$_POST['semester']);
                  //die("hii");
                     if(empty($details_stu['academic_id'])){
                        $this->_refresh(3,'/academic/application/tuitionfees?v2','You are Not Promoted Please Consult college Department..!');
                }
//                else {
...
//                }
                 $currentdate= date('Y-m-d');
                 if($details_stu['session_id'])
                   $getperiod = $tut_fees->getRecordByTermsession($_POST['semester'],$_POST['department'],$details_stu['session_id']);
                   //=====checking for fine dates 
                   
                   if(!$getperiod['session_id']){
                      // echo "<pre>";print_r($details_stu);exit;
                         $this->_refresh(5,'/academic/application/tuitionfees?v2','Fee not found..');
                    }
                  // print_r($getperiod['session_id']); die(); 
            ...
            //    die("hii");
                 if(!$details_stu['academic_id']){
                        ...
                        $this->_refresh(5,'/academic/application/tuitionfees?v2','You are Not Promoted Please Consult college Department..!');
                     }
        ...
        $fee = array_filter($fee,function($value){return ($value !== null && $value !== false && $value !== '' && $value !=0);});
                   ...
                   // echo "Oops Site has been disable due to technical issue will resume soon ..";exit;
                    ...
                    $insertdata = array (
                        ...
                        $departmentname = $dept_model->getRecord($_POST['department'])['department'];
 ...
 if($_POST['semester'] != 't1'){
                        ...
                        $transactionRequest->setCustomerBillingAddress($_POST['stu_id']."|".$des."|". $departmentname);
                        $transactionRequest->setCustomerId($_POST['exam_id']."|".$_POST['class_roll']."|".$_POST['semester']);
                        $transactionRequest->setCustomerAccount("639827");
                      $transactionRequest->setReqHashKey("b338c08a991313f12c");
...
//echo "<pre>"; print_r($result['session_id']);exit;
	...
	public function marksheetAction(){
        ...
        public function finalyearmarksheetAction(){
          ...
          if($_GET['msg'])  {
           $this->_refresh(5,'/academic/application/finalyearmarksheet','You Have Choose Wrong Batch OR please Contact concerned Office for your Marksheet..!');
   ...
   public function admitcardprintAction() {
       ...
       // $assignment_form = new Application_Form_AdmitCardView();
        ...
        $student_exam_form = new Application_Model_ExamformSubmissionModel();
        $examschedule_model = new Application_Model_ExamDateModel();
        $ec_id = $this->_getParam("id");
...
//                   }
               ...
               // echo "<pre>"; print_r($checkk);exit;
                  ...
                  // die();
                   ...
                   $exam_schedule = $examschedule_model->getRecordBysessionType($result['session_id'],$semester,$acad_term_arr['batch_id'],1);
                   $this->view->exam_sch = $course_details_arr;
                  ...
                  //echo $exam_schedule[0]['course_id'];
                  //die();
                    $result['comm_exam'] = date('M-Y',strtotime($exam_schedule[0]['exam_date']));
                  //  echo "<pre>";  print_r($result['comm_exam']);die;
                    $paymentdesc = new Application_Model_ExamfeeSubmitModel();
                    ...
                    $this->view->bankname = $fetchdetail['bank_name'];
                    $this->view->term_id = $acad_term_arr['term_id'];
                    $this->view->examdate = $result['comm_exam'];
                    $assignment_form->populate($result);
       ...
       // $assignment_form = new Application_Form_AdmitCardView();
        ...
        //$this->view->form = $assignment_form;
		     ...
		     ///die();
                    ...
                    $this->view->fid = $s_id;
                    $result =$student_exam_form->getNonRecordbyfid($s_id,$term);
                    $exam_roll = $result['examination_id'];
                 ...
                 if(!$ac_details || !$ac_details['department']){
                   ...
                   $result['registration_no'] = $result['reg_no'];
                    ...
                    $this->view->transaction = $fetchdetail['mmp_txn'];
                    $this->view->bankname = $fetchdetail['bank_name'];
                    $exam_schedule = $examschedule_model->getRecordBysessionType($result['session_id'],$term,$acad_term_arr['batch_id'],2);
                   $this->view->exam_sch = $course_details_arr;
                  //echo "<pre>";  print_r($exam_schedule);
                  ...
                  //die();
                    $nonsubject = new Application_Model_BackSelectionItems();
                     $getsub= $nonsubject->getStuSubRecords($s_id,$term);
                     $this->view->subjectdetail = $getsub;
                    $result['comm_exam'] =date('M-Y',strtotime($exam_schedule[0]['exam_date']));
                    $this->view->term_id = $acad_term_arr['term_id'];
                    $this->view->examdate = $result['comm_exam'];
      ...
      public function admitcardpgnoncollegiateprintAction() {
       ...
       // $assignment_form = new Application_Form_AdmitCardView();
        ...
        //$this->view->form = $assignment_form;
                    ...
                    $result =$student_exam_form->getNonRecordbyfid($st_id,$term);
                    $exam_roll = $result['examination_id'];
                   // print_r($result);
                      // die();
...
//echo "<pre>"; print_r($result['session_id']);exit;
    ...
    //http_response_code(404); die;
        ...
        $paymentsem = new Application_Model_FeesCollection();
        ...
        $transactionRequest  = new TransactionRequest();
            ...
            switch ($type) { 
                ...
                case "view": 
                    // if($_POST['stu_id']!='F-2019-6585'){
                    //   $this->_refresh(5,'/academic/application/transfer-cert-fees?v2','Technical Issue Please visit After 3 PM..');  
                    // }
                   ...
                   // die();
                    ...
                    $stu_id = trim(str_replace(")","",$_POST['stu_id']));
                    
                     $applicantDetails = $studetails->getStudenInfoByFormId($stu_id);
                     if(!$applicantDetails)
                      $this->_refresh(5,'/academic/application/transfer-cert-fees?v2','error code:500 Leaving sem is not correct..');
                   // echo '<pre>'; print_r($applicantDetails); exit;
                        $Grade_Model = new Application_Model_GradeAllocationItems();
                       // echo "<prE>";print_r($stu_id);exit;
                        $result = $Grade_Model->getcompletedterms($stu_id);
                        $completed_year = (int)$result['completed_year'];
                        $numofterm = $completed_year*2;
                        $terms = array();
                        for($i = 1;$i<=$numofterm; $i++){
                        $terms[$i] = 't'.$i;    
                        }
                        $cmnterms =  "'".implode("','",$terms)."'";
                        $marksDetails = $Grade_Model->getCGPAForStudent($applicantDetails['academic_id'],$stu_id,$cmnterms);
                    ...
                    $this->view->cgpa = $marksDetails['cgpa'];
                ...
                case "save":
                  //    echo '<pre>'; print_r($_POST['stu_id']);exit;
                    ...
                    $details_stu = $studetails->getStudenacademicDetailsnew(trim($_POST['stu_id']));
                    
                    //echo '<pre>'; print_r($details_stu); exit;
                  if( $_POST['type'] == 'tc' && $details_stu['stu_status']==3){
                        $this->_refresh(5,'/academic/application/transfer-cert-fees?v2','You had already paid and Collect selected Certificate Type OR Please Contact Your College Office. ..');
                    }
                    else if($details_stu['is_migration']==1)
                    {
                        $this->_refresh(5,'/academic/application/transfer-cert-fees?v2','You had already paid and Collect selected Certificate Type OR Please Contact Your College Office. ..');
                    }
                    ...
                    if(!empty($checkPayment['pay_status'])){
                        ...
                        foreach($seeionid as $key=>$value) {
                       if($_POST['type'] == 'tc'  && $value['id']==$details_stu['session_id']){
                          
                             
                             $leave =explode('-',$details_stu['session']);
                             
                            $lastterm=  $leave['1']-$leave['0'];
                        
                            if($lastterm=='3'){
                                $t='t6';
                                 $checklastpay=$paymentsem->getPayRecords($_POST['stu_id'],$t);
                            }else if($lastterm=='2') {
                                $t='t4';
                                 $checklastpay=$paymentsem->getPayRecords($_POST['stu_id'],$t);
                            }else if($lastterm=='1') {
                             ...
                             /// echo '<pre>'; print_r($checklastpay); exit;
                            if(!empty($checklastpay)){
                            ...
                            $payment->update($updatedata, array('id=?' => $last_insert_id)) ;
                        $this->_refresh(5,'/academic/application/transfer-cert-fees?v2','You had already paid the fees for selected Fee Type. ..');
                    }
    ...
    public function tcfeeresponseAction(){
      ...
      /// comment below code rausha 06-06-2023//
       // $transactionResponse = new Application_Model_TransactionResponse();
...
//
//                }
                $details_stu = $studetails->getStudenacademicDetailsnew(trim($record['stu_id']));
        }
        else{
            $record= $payment->getRecord($id);
            $this->view->response_data = $record;
    ...
    //http_response_code(404); die;
        ...
        $payment = new Application_Model_TcFeeCollection();
        $paymentsem = new Application_Model_FeesCollection();
        $backselection = new Application_Model_BackSelectionItems();
        $session =  new Application_Model_Session();
        $type = $this->_getParam("type");
        ...
        $transactionRequest  = new TransactionRequest();
            ...
            switch ($type) { 
                ...
                case "view": 
                    ...
                    $feeType = "Convocation";
                    $_SESSION['feeType'] = "Convocation";
                    
                    
                    
                    $date = "2023-05-02"; // replace with the date you want to check
                    $enddate = "2023-05-20";

$tomorrow = strtotime($date);
$twenty_days_from_now = strtotime($enddate);
$current_date = date('Y-m-d');
$timestamp = strtotime($current_date);

if ($timestamp >= $tomorrow && $timestamp <= $twenty_days_from_now) {
                    ...
                    $stu_id = trim(str_replace(")","",$_POST['stu_id']));
                  //  326
                     $applicantDetails = $studetails->getStudenacademicDetailsnew($stu_id);
                     $pendingstudents = $studetails->getpendingstudent($stu_id);
                     //passed students after 30-04-2023
                     $passedstudent = $backselection->getStudRecordsbydate($stu_id);
                     if(!$applicantDetails)
                      $this->_refresh(5,'/academic/application/transfer-cert-fees?v2','Form Id is incorrect');
                      
                      if(!in_array($applicantDetails['session_id'],array(3,2,6)) && ($applicantDetails['department'] !=30 && in_array($applicantDetails['session_id'],array(1,8))) )
                      $this->_refresh(5,'/academic/application/convocation?v2','Form Id is not available for convocation');
                   
                   if($pendingstudents)
                   $this->_refresh(5,'/academic/application/convocation?v2','pending students are not allowed for convocation');
                   ...
                   $this->_refresh(5,'/academic/application/convocation?v2','Form Id is not available for convocation');
                    ...
                    $this->view->fee_type=$feeType;
                  $this->view->stud_info=$applicantDetails;
        
                    
                    
                    
                    if(!empty($details_stu['pay_status'])){
                        $this->_refresh(5,'/academic/application/convocation?v2','You had already paid the fees for convocation fee. ..');
                    }
                    
}
else
$this->_refresh(5,'/academic/application/convocation?v2','Not Started or closed..');
                break;
                case "save":
                  //    echo '<pre>'; print_r($_POST['stu_id']);exit;
                     $updateData = array (
                    ...
                    if(!empty($checkPayment['pay_status'])){
                       ...
                       // echo '<pre>'; print_r($_POST); exit; 
                        ...
                        $transactionRequest->setTransactionCurrency("INR");
                        $transactionRequest->setTransactionAmount($payable_amount);
                        $transactionRequest->setReturnUrl($this->_base_url  . 'application/convofeeresponse/?id='.$last_insert_id);
                        $transactionRequest->setClientCode(53243);
                        $transactionRequest->setTransactionId($transactionId);
                        $transactionRequest->setTransactionDate($transactionDate);
                        $transactionRequest->setCustomerName($details_stu['stu_fname']);
                        $transactionRequest->setCustomerEmailId($_POST['stu_email_id']);
                        $transactionRequest->setCustomerMobile($_POST['phone']);
                        $transactionRequest->setCustomerBillingAddress($_POST['stu_id']."|Convocation");
                        $transactionRequest->setCustomerId($details_stu['exam_roll']."|".$details_stu['roll_no']);
                        $transactionRequest->setCustomerAccount("639827");
                      $transactionRequest->setReqHashKey("b338c08a991313f12c");
                      $transactionRequest->seturl("https://payment.atomtech.in/paynetz/epi/fts");
      ...
      public function convofeeresponseAction(){
        require_once APPLICATION_PATH . '/public/atompay/TransactionResponse.php';
...
//     'param1' => 'value1',
//     'param2' => 'value2'
// ); // Query parameters to pass in GET request
