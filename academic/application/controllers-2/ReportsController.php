...
//        }
			...
			$StudentPortal_model = new Application_Model_ElectiveSelectionItems();
                        ...
                        $app_course_details = new Application_Model_ApplicantCourseDetailModel();
                        ...
                        else if($col == 2)
                        {
                            ...
                            if($course_id !=0){
                            $result = $backStudentElective->getelectivestudentDetails1($academic_year_id,$term_id,explode('_',$course_id)[0],1);
                            }
                            else{
                             $course =   $course_model->getCourseCode($course_code);
                             $result =    $backStudentElective->getelectivestudentDetails1($academic_year_id,$term_id,$course['course_id'],0);
                            }
                        } else if($col == 3){
                          //  echo "<pre>"; print_r($academic_year_id);exit;
                            $result = $app_course_details->getRecordByCourse(explode(',',$academic_year_id));
                            }
      ...
      $this->_helper->layout->disableLayout();
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
             $session_id = $this->_getParam("session_id");
              ...
              $department = $dept->getByDepartmentType($department_id);
              $academic_details = $academic->getAcademicOnDept($department['did'],$session_id,$degree_id,'');
              echo "<option>--select--</option>";
              foreach($academic_details as $key => $value){
                  echo "<option value ='".$value['academic_year_id']."'>".$value['batch_code']."</option>";
...
public function ajaxGetNonpgDetailsAction() {
    ...
    public function getAttendanceReportAction(){
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            //echo '<pre>';print_r($practical_id);exit;
