...
//                $masterIds =implode(',',$batchData);
                ...
                foreach ($academic_id_Arr as $key => $value) {
                    if ($value) {
                     ...
                     //   echo $ct_id;
                       ...
                       //  $raw_data[$value]   = $StudentPortal_model->getstudentsdetailattendnewModified($value,$term_id,$pay,$tabId,$termId['term_id'],$section,"'".implode($checkInData['course_id'],"','")."'"); 
 ...
 //echo '<pre>';print_r($Category_data);
                ...
                foreach ($Category_data as $key => $value) {
                    if($readd){
                         $acadyear =    explode("-",$value['stu_id'])[1];
                         $currentregno = substr($acadyear,2)."PWC";
                        
                         if(substr($value['reg_no'],0,5) == $currentregno)
                         continue;
                    }
                  //  echo '<pre>';print_r($checkInData);exit;
