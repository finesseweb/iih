...
// ramesh for enabled between dates in year 
    ...
    public function getFailRecordAccordingTotabulations($session,$term){
      $select ="SELECT erp_student_information.stu_fname,erp_student_information.reg_no,erp_student_information.student_id,erp_student_information.stu_id,erp_student_information.exam_roll,course_master.course_id,ge_id,course_master.course_code ,term_master.term_id,erp_student_information.academic_id FROM `academic_master`
            ,tabulation_report
            ,tabulation_report_items
            ,erp_student_information
            ,term_master ,course_master
            ,core_course_master
            WHERE 
            erp_student_information.student_id = tabulation_report_items.student_id
            and tabulation_report.tabl_id = tabulation_report_items.tabl_id
            and term_master.academic_year_id = academic_master.academic_year_id
            and tabulation_report.term_id = term_master.term_id
            and core_course_master.course_id = course_master.course_id
            and term_master.term_id = core_course_master.term_id
            and FIND_IN_SET(course_master.course_id, tabulation_report_items.course_id)
            and term_master.cmn_terms in $term
            and tabulation_report_items.sgpa =0
            and academic_master.`session` = $session  
            ORDER BY `erp_student_information`.`stu_id` ASC";
$result = $this->getAdapter()
            ->fetchAll($select);
