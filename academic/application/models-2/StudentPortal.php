...
$result = $this->getAdapter()
    ...
    public function getStudenacademicDetails($id) {
        ...
        $select = $this->_db->select()
            ->from($this->_name)
         ->joinLeft(array("academic_master"), "academic_master.academic_year_id=$this->_name.academic_id", array("short_code","academic_year"))
...
//echo $select; die;
        ...
        $select = $this->_db->select()
            ->from($this->_name)
         ->joinLeft(array("academic_master"), "academic_master.academic_year_id=$this->_name.academic_id", array("short_code","department","academic_year"))
         ->joinLeft(array("session_info"), "session_info.id=academic_master.session", array("session","id as session_id"))
...
///print_r($result); die();
        ...
        /// 05-06-2023  raushan ///
        if($result['name_of_university_exam']) {
...
//=====comented
  ...
  public function getRecordsBySessionselected($session_id = '', $department_id = '', $flag = '') {
        ...
        "cast_category", "institution", "university", "last_exam_year_passing", "place_of_exam","l_state as state","religion","is_migration",
        "l_homeTown","l_postOffice","l_city","l_pin_code","p_homeTown","p_postOffice","p_police_station","p_city","p_pin_code","p_state","l_police_station"));
...
public function getRecordsForEarnedcreditnew($session_id = '', $department_id = '', $flag = '') {
   $cmn_terms = $this->getSecondLastTerm($session_id , $department_id );
    ...
    public function getstudentsdetailattendnewModified($academic_id, $term_id = '',$pay = false, $tab_Offset = true,$termMasterId='',$section='',$course) {
       ...
       // echo '<pre>'; print_r($term_id);exit;
        ...
        if ($term_id != 't1') {
            ...
            $term_id = $currentTerm;
             ...
             $term_model = new Application_Model_TermMaster();
           $term_id1 = $term_model->getTermRecordsbycmn($academic_id, $currentTerm)['term_id'];
           // $term_id1 = $termMasterId-1;
      
        if ($tab_Offset != 1) 
         $term_id = $currentTerm;
//echo '<pre>'; print_r($term_id1);exit;
        $select = "select newtb.*,core_course_master.* from (SELECT erp_student_information.*   FROM 
                    `erp_student_information`";
                    if ($tab_Offset == 1) {
            if ($currentTerm != 't1') {
                $select.="  ,tabulation_report
                    ,tabulation_report_items";
            }
                    }
                    if($pay){
              $select.="  ,exam_form_submission";
               $select.=" ,term_master";
               }
                  $select.=" WHERE"; 
                    if($pay)
                   $select.=" term_master.academic_year_id = erp_student_information.academic_id";
                    if ($tab_Offset == 1) {
            if ($currentTerm != 't1') {
               $select.="   and tabulation_report.academic_id = erp_student_information.academic_id
                    and tabulation_report.term_id = term_master.term_id
                    and tabulation_report_items.tabl_id = tabulation_report.tabl_id
                    and tabulation_report_items.student_id = erp_student_information.student_id
                    and tabulation_report_items.final_remarks !='F'
                    and tabulation_report_items.promotion_text != ''";
            }}
                  
                   if($pay){
                       $select.="  and exam_form_submission.student_id = erp_student_information.student_id";
                  $select.="  and exam_form_submission.payment_status = 1
                    and exam_form_submission.term_id = term_master.term_id
                  ...
                  $select.=" 
                      erp_student_information.academic_id = $academic_id) as newtb
                    ,core_course_master
                    where core_course_master.ge_id in (newtb.stu_ge,24,25)
                    and core_course_master.academic_year_id = newtb.academic_id
                    and core_course_master.course_id in ($course);
                    and core_course_master.term_id = $term_id1";
//echo "<pre>".$select; die;
        $result = $this->getAdapter()
            ->fetchAll($select);
...
ORDER BY `cgpa` DESC";
  ...
  public function getStudentResultnew($academic_id = '', $term_ids = '',$tot=6) {
        ...
        if (!empty($academic_id)) {
            ...
            $select = "SELECT erp_student_information.stu_id,erp_student_information.student_id, erp_student_information.stu_fname,tabulation_report.added_date,erp_student_information.academic_id FROM `tabulation_report` 
JOIN tabulation_report_items on tabulation_report_items.tabl_id = tabulation_report.tabl_id
join erp_student_information on erp_student_information.student_id = tabulation_report_items.student_id
where tabulation_report.academic_id = $where and tabulation_report_items.sgpa !=0  and tabulation_report.term_id in ($term_ids) GROUP by tabulation_report_items.student_id having count(*) = $tot
";
            // $select = "SELECT `erp_student_information`.* FROM `erp_student_information` WHERE erp_student_information.status!=2 $where GROUP BY erp_student_information.student_id order by erp_student_information.exam_roll";
        }
