 ...
 */
class Application_Model_AddonEmployeeAllotment extends Zend_Db_Table_Abstract
{
    public $_name = 'employee_addon_allocation';
    protected $_id = 'id';
    ...
    {       
        $select=$this->_db->select()
                ->from($this->_name)
                ->joinleft(array("addon"=>"addon_course_mater"),"addon.id=$this->_name.addon_course_id",array("name","id as addon_id"))
                ->joinleft(array("year"=>"academic_year"),"year.year_id=$this->_name.academic_year",array("academic_year","year_id"))
...
public function getEmployeeTermsNew($course_type, $employee_id){
         $select = $this->_db->select()
	->from($this->_name)
	->joinLeft(array("addon_course"=>"addon_course_mater"),"addon_course.id=$this->_name.addon_course_id",array("name"))                                     
	->where("$this->_name.addon_course_id=?",$course_type)
	->where("$this->_name.status != 2")
       ...
       //  echo $select; die();
		...
		->fetchAll($select);
		 ...
		 $select = $this->_db->select()
					->from($this->_name)
					//->joinLeft(array("allocation_items"=>"employee_addon_allocation"),"allocation_items.ea_id=$this->_name.ea_id",array('term_id','course_id','department_id','employee_id'))
					//->joinLeft(array("term"=>"term_master"),"term.term_id=allocation_items.term_id",array("term_id as term","term_name"))
					->joinLeft(array("course"=>"course_master"),"course.course_id=allocation_items.course_id",array("course_id as course","course_name"))	
					->where("$this->_name.academic_year_id=?",$academic_year_id)
					//->where("allocation_items.department_id=?",$department_id)
...
/*  public function getComponentCourses($academic_year_id,$department_id,$employee_id,$term_id){
  ...
  public function getExistedDataByYear($academic_year_id)
	{
		$select = $this->_db->select()
			  ->from($this->_name)
...
public function getRecordByAddonId($addonId,$acadId)
    {       
        ...
        $select=$this->_db->select()
         ->from($this->_name) 
	 ->joinleft(array("addon"=>"addon_course_mater"),"addon.id=$this->_name.addon_course_id",array("name","id as addon_id"))
         ->joinleft(array("year"=>"academic_year"),"year.year_id=$this->_name.academic_year",array("academic_year"))
         ->where("$this->_name.addon_course_id =?",$addonId)
         ->where("$this->_name.academic_year =?",$acadId)
    ...
    public function trashItems($ea_id) {
         $this->_db->delete($this->_name, "addon_course_id=$ea_id");
    }
}
?>