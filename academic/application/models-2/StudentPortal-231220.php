...
class Application_Model_StudentPortal extends Zend_Db_Table_Abstract {
    ...
    public function getStudenInfoByStuIdForSemFee($id){

        $select=$this->_db->select()

        ->from($this->_name)
        ->joinleft(array("am"=>"academic_master"),"am.academic_year_id= $this->_name.academic_id",array('short_code','department','session'))
        ->where("$this->_name.status !=?", 2)
