...
class Application_Model_GradeAllocation extends Zend_Db_Table_Abstract {
    ...
    public function getDistinctmonthofdeletedgrdes($academic_year_id, $term_id) {
        $select = $this->_db->select()
                ...
                ->where("$this->_name.term_id in (?)", $term_id)
    ...
    //    echo $select; die;
        ...
        public function getDistinctmonthofaccugrdes($academic_year_id) {
        $select = $this->_db->select()
                ->from($this->_name, array())
                ->join(array("deleted_grade" => "accu_grade_allocation_items"), "deleted_grade.grade_allocation_id=$this->_name.grade_id", array("DATE_FORMAT(deleted_grade.publish_date,'%d %b %Y') as display", "DATE_FORMAT(deleted_grade.publish_date,'%Y-%m-%d') as vald"))
                ->where("$this->_name.academic_id in (?)", $academic_year_id)
                ->group("DATE_FORMAT(deleted_grade.publish_date,'%Y-%m-%d')");
        //->where("$this->_name.status !=?", 2);
   // echo "<pre>".$select; die;
    ...
    public function getGrouped($session, $faculty, $term, $course, $paper = 'R',$month="") {
        $select = $this->_db->select();
...
//echo $select ; die;
    ...
    public function isGradeAllocated1($batch_id, $faculty_id, $term_id, $course_id, $allocated = 'R',$month="") {
        $select = $this->_db->select();
