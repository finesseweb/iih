...
class Application_Model_Aluminiassociation extends Zend_Db_Table_Abstract {

    public $_name = 'alumini_association';
    protected $_id = 'id';
    ...
    public function getRecordByEmailID($id,$mob){
                   ...
                   $select = $this->_db->select()
                      ->from($this->_name)
                      ->where("$this->_name.email_id=?", $id)
                      ->where("$this->_name.mobile=?", $mob)
