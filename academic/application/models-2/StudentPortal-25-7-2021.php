...
class Application_Model_StudentPortal extends Zend_Db_Table_Abstract {
    ...
    public function getelectivestudentDetailsBack($academic_id = '', $term_id = '', $pay = false,$month="") {
...
//echo $select;die;
    ...
    public function getstudentsbyacademics($academic_id, $term_id, $pay = false, $attendance = false) {
        ...
        $select->where("$this->_name.status !=?", 2);
        $select->where("$this->_name.stu_status not in (?)", array(2,3));
        if ($pay) {
            $select->where("payment.payment_status =?", 1);
...
//=====comented
    ...
    public function getstudentsByStuStatus($academic_id) {
        ...
        $select = $this->_db->select()
            ->from($this->_name, array('CONCAT(erp_student_information.stu_fname," ",erp_student_information.stu_lname) AS students', 'erp_student_information.student_id', 'father_fname', 'roll_no', 'exam_roll', 'stu_id', 'stu_status','reg_no','stu_email_id','father_mobileno'))
            ->where("$this->_name.academic_id=?", $academic_id);
