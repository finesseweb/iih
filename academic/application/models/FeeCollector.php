...
class Application_Model_FeeCollector extends Zend_Db_Table_Abstract {
	...
	public function get_student_record2($year, $session, $batch,$sem,$yearcm='',$cast='') {
		$select = $this->_db->select();
		$select->from("erp_student_information", array("erp_student_information.*", "student_id as stid"));
		$select->join(array("academic_master"), "academic_master.academic_year=erp_student_information.year_id");
		$select->join(array("department"), " department.id=academic_master.department");
	...
	public function get_student_paid($stid,$term_id) {
		$select = $this->_db->select();
		$select->from('fee_collector',array("id","discount","SUM(total_paid) as totalpaid","extra_charges"));
		$select->where("student_id=?", $stid);
	...
	public function getTotalpaidfeebystu($stid,$term_id,$acad) {
		...
		$select = $this->_db->select();
		$select->from('fee_collector',array("id","discount","extra_charges"));
		$select->where("student_id=?", $stid);
		$select->where("sem_year=?", $term_id);
	...
	public function getCollectionWiseFilteredStudentRecord($academic_id, $term_id,$filter_status){
        $select = $this->_db->select()
            ->from($this->_name)
           ->joinleft(array("erp_student_information"),"erp_student_information.student_id=$this->_name.student_id")
		   ->where("$this->_name.academic_year_id=?", $academic_id)
		   ->where("$this->_name.sem_year=?", $term_id)
		   ->group("$this->_name.student_id");
           // echo $select;die;
