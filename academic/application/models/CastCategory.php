 ...
 */
class Application_Model_CastCategory extends Zend_Db_Table_Abstract {

    public $_name = 'cast_category';
    protected $_id = 'id';
    ...
    public function getRecordByAcademicId($academic_id) {
	 ...
	 public function getRecordAllAcademicId($academic_id) {
        ...
        public function getRecordBysessionIdonly($session_id) {
        $select = $this->_db->select()
            ->from($this->_name,array("$this->_name.*","max(year_id) as last_year_id"))
    ...
    public function getactiveByAcademicId($academic_id) {
        $select = $this->_db->select()
            ->from($this->_name)
            ->join(array("examination_dates") , "examination_dates.academic_id=$this->_name.academic_year_id")
            ->where("academic_year_id=?", $academic_id)
            ->where("$this->_name.status !=?", 2)
            ->where("examination_dates.academic_id =?", $academic_id)
             ->order("$this->_name.cmn_terms");
        $result = $this->getAdapter()
            ->fetchAll($select);
        return $result;
    ...
    public function getRecordsByBatch($id) {
        $select = $this->_db->select()
            ->from($this->_name)
            ->joinleft(array("academic" => "academic_master"), "academic.academic_year_id=$this->_name.academic_year_id", array("short_code", "from_date", "to_date"))
    ...
    public function getValidateTermNameById($term_id) {
        ...
        $select = $this->_db->select()
            ->from($this->_name, array("term_name", "term_id","session"))
    ...
    public function getTermName($term) {
        ...
        $select = $this->_db->select()
            ->from($this->_name, array("term_name", "term_id","cmn_terms"))
	...
	public function getCastDropDownList() {
        $select = $this->_db->select();
        $select->from($this->_name, array('id', 'name',));
        $select->joinLeft(array("erp_fee_structure_master"), "erp_fee_structure_master.cast_category=$this->_name.id", array());
        $select->joinLeft(array("academic_master"), "academic_master.academic_year_id=erp_fee_structure_master.academic_id", array());
...
//print_r($result); die();
    ...
    public function getTermRecordsbycmn($academic_year_id, $term_id) {
        ...
        $select = $this->_db->select()
            ->from($this->_name, array('term_id', 'term_name', 'term_description',"session"))
            ->where("$this->_name.status != 2")
            ->where("$this->_name.academic_year_id in (?)", $academic_year_id)
            ->where("$this->_name.cmn_terms=?", !is_array($term_id)?$term_id:implode(",",$term_id))
            ->orwhere("$this->_name.term_id in (?)", $term_id);
   //     echo $select; die();
        $result = $this->getAdapter()
            ->fetchRow($select);
    ...
    public function getAcademicTermIdAction($academic_year_id, $term_id) {
        ...
        $select = $this->_db->select()
            ->from($this->_name, array('min(term_name) as term_name'))
            ->where("$this->_name.status != 2")
            ->where("$this->_name.academic_year_id=?", $academic_year_id)
            ->where("$this->_name.cmn_terms >?", $term_id);
          ///  echo $select; die();
    ...
    public function getTermId($academic_year_id, $term_id) {
    ...
    public function getNonTermRecordsByYear($academic_year_id, $year_id) {
        ...
        $select = $this->_db->select()
            ->from($this->_name)
            ->join(array("nonug" => "back_selection_items"), "nonug.terms=$this->_name.term_id")
            ->where("$this->_name.status != 2")
            ->where("$this->_name.academic_year_id=?", $academic_year_id)
            ->where("$this->_name.year_id=?", $year_id)
    ...
    public function getNonpgTermRecordsByYear($academic_year_id, $year_id) {
        ...
        $select = $this->_db->select()
            ->from($this->_name)
            ->join(array("nonpg" => "back_selection_items"), "nonpg.terms=$this->_name.term_id")
            ->where("$this->_name.status != 2")
            ->where("$this->_name.academic_year_id=?", $academic_year_id)
            ->where("$this->_name.year_id=?", $year_id)
        ...
        public function departmentOnTerm($term,$session) {
        $select = $this->_db->select()
            ->from($this->_name,array(''))
            ->join(array("academic_master" => "academic_master"), "academic_master.academic_year_id=$this->_name.academic_year_id",array(''))
            ->join(array("department" => "department"), "department.id=academic_master.department",array(''))
            ->join(array("department_type" => "department_type"), "department_type.id=department.department_type",array('id','department_type'))
            ->where("$this->_name.status != 2")
            ->where("$this->_name.cmn_terms=?", $term)
            ->where("academic_master.session=?", $session)
            ->group("department.department_type");
    ...
    public function getTermRecordsByTerm1($academic_year_id, $term_id) {
...
//Sailaja
    ...
    public function getTermOnAcademicId($batch) {
        ...
        $select = $this->_db->select()
            ->from($this->_name, array('academic_year_id', 'term_id', 'start_date', 'end_date', 'term_name'))
            ->where("academic_year_id in (?)", $batch)
            ->where("$this->_name.status != ?", 2);
        $result = $this->getAdapter()
            ->fetchAll($select);
    ...
    public function getCourseReport($course_id, $batch_id, $term_id, $section='', $version = '') {
        $select = $this->_db->select()
           ...
           $select->where("batch_id=?", $batch_id);
            $select->where("course_code=?", $course_id);
            if(!empty($section))
    ...
    public function getYearTermIds($acadId, $yearId) {
        $select = $this->_db->select()
            ->from($this->_name)
            ->where("$this->_name.academic_year_id=?", $acadId)
            ->where("$this->_name.year_id=?", $yearId)
            ->where("$this->_name.status != ?", 2);
        $result = $this->getAdapter()
            ->fetchAll($select);
    ...
    public function getRecordBySessionId($session_id,$cmnterms) {
        $select = $this->_db->select()
            ->from($this->_name)
           ->joinleft(array("academic" => "academic_master"), "academic.academic_year_id=$this->_name.academic_year_id", array("short_code", "from_date", "to_date"))
           ->where("academic.session =?", $session_id)
            ->where("$this->_name.cmn_terms =?", $cmnterms)    
            ->where("$this->_name.status !=?", 2)
            ->order("$this->_name.cmn_terms");
     //  echo $select; die(); 
        $result = $this->getAdapter()
