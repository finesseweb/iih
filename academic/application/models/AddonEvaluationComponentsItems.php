 ...
 */
class Application_Model_AddonEvaluationComponentsItems extends Zend_Db_Table_Abstract
{
    public $_name = 'addon_evaluation_componets_master_items';
    protected $_id = 'id';
...
public function  getWeightage($addon_id,$employee_id,$department_id,$course_id){
		$select = $this->_db->select()
				->from($this->_name)
				->joinleft(array("component"=>"addon_evaluation_componets_master"),"component.id=$this->_name.ec_id",array("addon_course_id","employee_id","department_id"))
				->where("component.addon_course_id=?",$addon_id)
...
public function  getWeightages($academic_year_id,$term_id,$course_id,$component_id){
		$select = $this->_db->select();
				...
				$select->joinleft(array("component"=>"addon_evaluation_componets_master"),"component.ec_id=$this->_name.ec_id",array("academic_year_id"));
	...
	public function getAlltermsandcourses($ec_id){
            if($ec_id){
		$select = $this->_db->select()
		           ->from($this->_name,array("GROUP_CONCAT(addon_evaluation_componets_master_items.course_id) AS courses"))
				   ->where("$this->_name.ec_id=?",$ec_id)
