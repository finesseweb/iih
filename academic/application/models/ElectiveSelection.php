 ...
 public function trashItems($student_id='',$term_id='') {
    ...
    public function getStudentsForElectiveByCourse($academic_year_id, $course, $term_id, $field_name = 'electives', $session = '', $ge_id = '', $join_attend = false, $pay = false, $tab_offset = false) {
     ...
     public function getNotAddedgeStudentInForMarks($academic_year_id, $course, $term_id, $field_name = 'electives', $session = '', $ge_id = '', $join_attend = false, $pay = false, $tab_offset = false){
       ...
       $select ="select * from (SELECT t.* FROM (SELECT temptb.*,grade_allocation_items.student_id as present,grade_allocation_master.grade_id as grade_id FROM (SELECT `elective_selection_items`.`students_id` AS `student_id`, 
       ...
       $select.=" FROM 
                 ...
                 AND (semester_wise_attendance_report.course_id IN ($course))":""; 
                   ...
                   AND (elective_selection_items.terms = ($term_id)) ) AS temptb
                    ...
                    LEFT join grade_allocation_items on grade_allocation_items.grade_allocation_id = grade_allocation_master.grade_id and grade_allocation_items.student_id = temptb.student_id ) as t GROUP by student_id,t.present) as numtb GROUP by student_id having count(*) = 1 and present is null";
  //  echo "<pre>".$select; die;
        $result = $this->getAdapter()
            ->fetchAll($select);
           // if(!count($result))
  //return  $this->getStudentsge($result,$academic_year_id, $course, $term_id, $field_name, $session, $ge_id, $join_attend, $pay, $tab_offset);
            return $result;
        
    }
    
    public function getStudentsge($result,$academic_year_id, $course, $term_id, $field_name = 'electives', $session = '', $ge_id = '', $join_attend = false, $pay = false, $tab_offset = false){
       $arr = array_unique(array_column($result,'grade_id'));
       if(count($arr)==1){
       ...
       $select.=" FROM 
                 ...
                 AND (semester_wise_attendance_report.course_id IN ($course))":""; 
                   ...
                   AND (elective_selection_items.electives IN($course)) 
                   AND (elective_selection_items.terms = ($term_id)) ) AS temptb
                    join grade_allocation_master on grade_allocation_master.academic_id = $academic_year_id and grade_allocation_master.course_id = temptb.course_id and grade_allocation_master.flag not like 'B' and grade_allocation_master.term_id =$term_id
                    LEFT join grade_allocation_items on grade_allocation_items.grade_allocation_id = grade_allocation_master.grade_id and grade_allocation_items.student_id = temptb.student_id ) as t where t.present is null ";
    ...
    public function getStudentsForElectiveByCourseAttend($academic_year_id, $course, $term_id, $field_name = 'electives', $session = '', $ge_id = '', $join_attend = false, $pay = false, $tab_offset = false, $termMasterId = '', $section = '') {
        ...
        //===kedar code comented $stu_status=['1','6'];
        $stu_status=['1'];
        $term_model = new Application_Model_TermMaster();
        $term_id1 = $term_model->getRecord($term_id)['cmn_terms'];
        ...
        if ($tab_offset == 1) {
            if ($currentTerm != 't1') {
                $select->join(array('tab_items' => 'tabulation_report_items'), "tab_items.student_id = $this->_name.student_id");
                $select->join(array('tab_report' => 'tabulation_report'), "tab_report.tabl_id = tab_items.tabl_id and tab_report.term_id = term.term_id");
            }
        }
        ...
        if ($tab_offset == 1) {
            if ($currentTerm != 't1') {
                ...
                $select->where("tab_items.final_remarks != ?", 'F'); //=======30-09-2022 un comment for ges;
...
// 		echo $select;exit;
    ...
    //To check only in elective selection
