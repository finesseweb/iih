...
class Application_Model_ApplicantCourseDetailModel extends Zend_Db_Table_Abstract {
    ...
    $exam_model = new Application_Model_EntranceExamScheduleModel();
        $select = $this->_db->select()
                ->from(array($this->_name), array('id','application_no',"applicant_name","form_id","session","degree_id","course"))
                ->where("md5($this->_name.application_no)=?", $application_no);
        $result = $this->getAdapter()
                ->fetchRow($select);
   
           $applicationnoarr =  explode("-",$result['application_no']);
           
           $select1 = 'SELECT CASE WHEN form_id IS null THEN  concat("F","-","'.$applicationnoarr[1].'","-","1") ELSE concat("F","-","'.$applicationnoarr[1].'","-",(max(CAST(substr(form_id,8,length(form_id)) as unsigned)) + 1)) end nextid 
           FROM `applicant_course_details` WHERE applicant_course_details.form_id like "F-'.$applicationnoarr[1].'%"';
            $resultid = $this->getAdapter()
                ->fetchRow($select1);
    
        
        if ($result) {
                $fee_details = $exam_model->getFee($result['session'],$result['degree_id'],$result['course']);
            $form_id = $resultid['nextid'];
            $where = array(
            ...
            $res['fom_id'] = $result['form_id'];
            $res['applicant_name'] = $result['applicant_name'];
            $res['amount'] =  $fee_details['examFee'];
            return $res;
        }
    }
