...
class Application_Model_BatachSemesterAttendance extends Zend_Db_Table_Abstract {
    ...
    public function checkRecordBycid($sid="" ,$id="",$term_id="")
    {       
        $select=$this->_db->select()
            ->from(array($this->_name))
            ->join(array("term"=>"term_master"),"term.cmn_terms=$this->_name.cmn_terms",array(""))
            ->where("$this->_name.u_id=?", $sid)
    ...
    public function getEndSemesterRecords($session,$term_id,$degree_id,$cc_id,$ge_id='',$hons_id='',$course_id=''){
		...
		$select->joinleft(array("students"=>"erp_student_information"),"students.stu_id=$this->_name.u_id",array('stu_status,CONCAT(students.stu_fname," ",students.stu_lname) AS studentName','roll_no','academic_id'));
		...
		if(!empty($course_id)){
			$select->where("$this->_name.course_id In(?)",$course_id);
			$select->where("$this->_name.cc_id =?",$cc_id);
    ...
    public function checkExitedRecord($data,$course_ids=''){
        //echo '<pre>';print_r($course_ids);exit;
        $select=$this->_db->select();
		$select->from(array($this->_name),array('conducted_class')) ;
		
		$select->where("$this->_name.session =?",$data['session']);
		//$select->where("$this->_name.section =?",$data['section']);
		$select->where("$this->_name.cmn_terms =?",$data['cmn_terms']);
		$select->where("$this->_name.degree_id =?",$data['degree_id']);
		
		if(!empty($data['department'])){
			$select->where("$this->_name.department =?",$data['department']);
			$select->where("$this->_name.cc_id =?",$data['cc_id']);
			$select->where("$this->_name.ge_id =?",$data['ge_id']);	
		}
		if(!empty($course_ids)){
			$select->where("$this->_name.course_id IN(?)",$course_ids);
			$select->where("$this->_name.cc_id =?",$data['cc_id']);
			$select->where("$this->_name.department =?",$data['department']);	
		}
        if(!empty($ge_id)){
    ...
    public function checkExitedStudentRecord($data,$formId){
      
        $select=$this->_db->select();
		$select->from(array($this->_name),array('u_id')) ;
		
		$select->where("$this->_name.session =?",$data['session']);
		//$select->where("$this->_name.section =?",$data['section']);
		$select->where("$this->_name.cmn_terms =?",$data['cmn_terms']);
		$select->where("$this->_name.degree_id =?",$data['degree_id']);
		$select->where("$this->_name.u_id =?",$formId);
		...
		if(!empty($data['course_id'])){
			$select->where("$this->_name.course_id IN(?)",$data['course_id']);
			$select->where("$this->_name.cc_id =?",$data['cc_id']);
			$select->where("$this->_name.ge_id =?",$data['ge_id']);		
		}
