 ...
 */
class Application_Model_AddonEvaluationComponents extends Zend_Db_Table_Abstract {

    public $_name = 'addon_evaluation_componets_master';
    protected $_id = 'id';
...
//print_r($result);die;					  
    ...
    public function getClassRoomRecordNew($id) {
        $select = $this->_db->select()
                ->from($this->_name)
                ->joinLeft(array("course" => "addon_course_master"), "course.course_id=$this->_name.course_id", array("course_id as course", "course_name","course_code"))
               ->joinleft(array("addon" => "addon_course_mater"), "addon.id=$this->_name.addon_course_id", array("name"))
                ->where("$this->_name.$this->_id=?", $id)
                ->where("$this->_name.status !=?", 2);
        $result = $this->getAdapter()
                ->fetchRow($select);
...
//print_r($result);die;					  
    ...
    public function getRecords() {
        $select = $this->_db->select()
                ->from($this->_name)
                ->joinLeft(array("components_items" => "addon_evaluation_componets_master_items"), "components_items.ec_id=$this->_name.id", array("GROUP_CONCAT(components_items.course_id) as courses", "GROUP_CONCAT(concat(component_name,' = ',weightage)) as components", "course_id", "component_id"))
               // ->joinleft(array("academic" => "academic_master"), "academic.academic_year_id=$this->_name.academic_year_id", array("from_date", "to_date", "short_code"))
                ->where("$this->_name.status !=?", 2)
    ...
    public function getItemRecordsNew($eval_component_id) {
        ...
        if ($eval_component_id) {
            ...
            $select = $this->_db->select()
                    ->from($this->_name)
                    ->joinInner(array("component_items" => "evaluation_components_items_master"), "component_items.ec_id=$this->_name.id", array("term_id", "course_id", "component_name", "weightage", "remaining_weightage"))
                    //->joinLeft(array("term"=>"term_master"),"term.term_id=component_items.term_id",array("term_name"))
                    ->joinLeft(array("course" => "course_master"), "course.course_id=component_items.course_id", array("course_id as course", "course_name"))
                    // ->joinleft(array("academic"=>"academic_master"),"academic.academic_year_id=$this->_name.academic_year_id",array("from_date","to_date","short_code"))
                    ->where("$this->_name.status != 2")
                    ->where("$this->_name.id=?", $eval_component_id);
    ...
    public function getExperientialEvaluationComponentsItemRecords($eval_component_id) {
        if ($eval_component_id) {
            $select = $this->_db->select()
                    ->from($this->_name)
                    ->joinInner(array("component_items" => "experiential_evaluation_components_items"), "component_items.ec_id=$this->_name.id", array("course_id", "component_name", "weightage", "remaining_weightage"))
    ...
    public function getAllComponentsonTerms($addon_id, $course_id) {
        ...
        $select = $this->_db->select()
                ->from($this->_name)
                ->joinLeft(array("component_items" => "addon_evaluation_componets_master_items"), "component_items.ec_id=$this->_name.id", array("ec_id", "course_id", "component_name", "weightage", "component_id"))
                ->where("$this->_name.status != 2")
                ->where("$this->_name.addon_course_id=?", $addon_id)
               // ->where("component_items.term_id=?", $term_id)
                ->where("component_items.course_id=?", $course_id);
        $result = $this->getAdapter()
                ->fetchAll($select);
    ...
    public function getAllComponentsonTerms1($academic_year_id, $addon_id) {
        ...
        $select = $this->_db->select()
                ->from($this->_name)
                ->joinLeft(array("component_items" => "addon_evaluation_componets_master_items"), "component_items.ec_id=$this->_name.id", array("ec_id", "course_id", "component_name", "weightage", "component_id"))
                ->joinLeft(array("core" => "core_addon_master"), "core.course_id=$this->_name.course_id")
                ->joinLeft(array("course_master" => "addon_course_master"), "course_master.course_id=$this->_name.course_id")
                ->where("$this->_name.status != 2")
                ->where("core.academic_year=?", $academic_year_id)
                ->where("core.course_id=?", $addon_id);
                 //->group('component_items.course_id')
                // ->order("core.course_id ASC");
        $result = $this->getAdapter()
                ->fetchAll($select);
        return $result;
    }
    ...
    public function getEvlComponentCountNew($course_id) {
                 $select = $this->_db->select()
                ->from($this->_name)
                 ->where("$this->_name.course_id=?", $course_id)
                ->where("$this->_name.status != 2");
