...
class Application_Form_AddonComponents extends Zend_Form
{
       ...
       // $data = $HRMModel_model->getEmployeeIds();
        $employee_id = $this->createElement('select','employee_id')
			...
			$this->addElement($employee_id);			
                        ...
                        $Academic_model = new Application_Model_AddonCourseModel();
        $data = $Academic_model->getDropDownList();
        //print_r($data); die;
        $academic_year_id = $this->createElement('select', 'addon_course_id')
                ->removeDecorator('label')
                ->setAttrib('class', array('form-control', 'chosen-select'))
                ->setAttrib('required', 'required')->setRequired(true)
                ->removeDecorator("htmlTag")
                ->addMultiOptions(array('' => 'Select Addon Courses'))
                ->addMultiOptions($data);
        $this->addElement($academic_year_id);
