...
class Finance_Model_TrailBalance extends Zend_Db_Table_Abstract {
	...
	{
		$select ="SELECT customer.dealer_id, (CASE WHEN dealer_type = '0' THEN customer.dealer_company_name WHEN dealer_type = '1' THEN  customer.custom_name END) AS name, customer.start_balance, customer.operator,(SELECT sum(waybill.total_amount) - sum(IF(receipt.payment_by = '1', waybill.total_amount, 0)) as total_amount FROM erp_sales_waybill as waybill LEFT JOIN finance_receipt_voucher AS receipt ON receipt.waybill_id=waybill.waybill_id LEFT JOIN erp_dealer_master ON erp_dealer_master.dealer_id=waybill.sales_dealer_id WHERE waybill.sales_dealer_id = customer.dealer_id and waybill.waybill_status !=2 and waybill.waybill_date >='".$data['start_date']."' and waybill.waybill_date <='".$data['end_date']."' GROUP BY waybill.sales_dealer_id) AS waybill_amount, 
		(SELECT sum(invoice.grand_total) - sum(IF(receipt.payment_by = '1', invoice.grand_total, 0)) as grand_total FROM erp_sales_invoice as invoice LEFT JOIN finance_receipt_voucher AS receipt ON receipt.invoice_id=invoice.sales_invoice_id LEFT JOIN `erp_dealer_master` AS cus ON cus.dealer_id=invoice.dealer_id WHERE invoice.dealer_id = customer.dealer_id and invoice.invoice_status !=2 and invoice.invoice_date >='".$data['start_date']."' and invoice.invoice_date <='".$data['end_date']."' GROUP BY invoice.dealer_id order by invoice.dealer_id) AS sales_amount,
		...
		(SELECT sum(db_voucher.debit_amount) FROM journal_voucher as db_voucher WHERE db_voucher.vendor_id = (CASE WHEN customer.dealer_type = '0' THEN customer.dealer_company_name WHEN customer.dealer_type = '1' THEN  customer.custom_name END) and db_voucher.status !=2 and db_voucher.date >='".$data['start_date']."' and db_voucher.date <='".$data['end_date']."' GROUP BY db_voucher.vendor_id) AS debit_amount,
		(SELECT sum(cd_voucher.credit_amount) FROM journal_voucher as cd_voucher WHERE cd_voucher.dealer_id = (CASE WHEN customer.dealer_type = '0' THEN customer.dealer_company_name WHEN customer.dealer_type = '1' THEN  customer.custom_name END) and cd_voucher.status !=2 and cd_voucher.date >='".$data['start_date']."' and cd_voucher.date <='".$data['end_date']."' GROUP BY cd_voucher.dealer_id) AS credit_amount
	...
	{
		$select ="SELECT vendor.vendor_id, vendor.vendor_name AS name, vendor.start_balance, vendor.operator, (SELECT sum(cinvoice.amount_inr) - sum(IF(payment.payment_by = '1', cinvoice.amount_inr, 0)) as amount_inr FROM erp_purchase_commercial_invoice as cinvoice LEFT JOIN finance_payment_vouvher AS payment ON payment.commercial_invoice_id=cinvoice.commercial_invoice_id LEFT JOIN erp_vendor_master ON erp_vendor_master.vendor_id=cinvoice.vendor_id WHERE cinvoice.vendor_id = vendor.vendor_id and cinvoice.commercial_invoice_status !=2 and cinvoice.invoice_due_date >='".$data['start_date']."' and cinvoice.invoice_due_date <='".$data['end_date']."' GROUP BY cinvoice.vendor_id) AS pcm_amount, 
		(SELECT sum(invoice.grand_total_amount) - sum(IF(payment.payment_by = '1', invoice.grand_total_amount, 0)) as grand_total_amount FROM erp_purchase_invoice as invoice LEFT JOIN finance_payment_vouvher AS payment ON payment.invoice_id=invoice.purchase_invoice_id LEFT JOIN erp_vendor_master ON erp_vendor_master.vendor_id=invoice.vendor_id WHERE invoice.vendor_id = vendor.vendor_id and invoice.purchase_invoice_status !=2 and invoice.invoice_date >='".$data['start_date']."' and invoice.invoice_date <='".$data['end_date']."' GROUP BY invoice.vendor_id) AS purchase_amount,
	...
	public function getAccountStatement($data)
	{
		...
		(SELECT sum(invoice.grand_total_amount) - sum(IF(payment.payment_by = '1', invoice.grand_total_amount, 0)) as grand_total_amount FROM erp_purchse_invoice_items AS purchase_items LEFT JOIN erp_purchase_invoice AS invoice ON invoice.purchase_invoice_id = purchase_items.purchase_invoice_no LEFT JOIN account_master AS ac ON ac.account_id=purchase_items.account_id LEFT JOIN finance_payment_vouvher AS payment ON payment.invoice_id=invoice.purchase_invoice_id WHERE  purchase_items.account_id=account.account_id and invoice.purchase_invoice_status!=2 and invoice_date >=".$data['start_date']." and invoice_date <='".$data['end_date']."' GROUP BY purchase_items.account_id) as purchase_amount,
