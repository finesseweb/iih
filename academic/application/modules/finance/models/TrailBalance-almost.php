...
class Finance_Model_TrailBalance extends Zend_Db_Table_Abstract {
	...
	public function getAccountStatement($data)
	{
		...
		FROM account_master AS account where account.status !=2 and cash_account !=1 order by account_name";	
		//echo $select; die;
		$result = $this->getAdapter()
                ->fetchAll($select);		
        return $result;			
	...
	{		
		...
		*/
		$select ="		
			...
			SELECT `id` as sales_invoice_id, DATE_FORMAT(date,'%Y-%m-%d') as date,debit_amount, null, `debit_amount` as sub_amount, `vendor_id` as vendor_name, (CASE WHEN `journal_voucher`.`payment_type` = '5' THEN '5' END) as payment_status,(CASE WHEN `payment_type` = '5' THEN 'JV' END) as payment_type , remark1 as invoice_increment_id FROM `journal_voucher` left join account_master on account_master.account_id=journal_voucher.debit_party_id and debit_party_type='A' WHERE  payment_type = '5' and account_master.vat_status='1' and date >='".$data['start_date']."' and date <='".$data['end_date']."'
