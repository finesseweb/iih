...
class Finance_Model_InvoiceStatement extends Zend_Db_Table_Abstract {
	...
	public function getAccountStatementByDates($data)
	{
		...
		if( !empty($data['party_id']) ){
			...
			$select ="SELECT sales_invoice.sales_invoice_id, DATE_FORMAT(sales_invoice.invoice_date,'%Y-%m-%d') as date, sales_invoice.grand_total AS `paid_amount`, `customer`.`dealer_company_name` as vendor_name, `sales_invoice`.`receipt_status` as payment_status, (CASE WHEN `sales_invoice`.`receipt_status` = '4' THEN 'SI' END) as payment_type, sales_invoice.invoice_increment_id, sales_invoice.name_of_transport, sales_invoice.no_of_packages, sales_invoice.despatched_through, sales_invoice.private_mark, null FROM `erp_sales_invoice` AS `sales_invoice` LEFT JOIN `erp_sales_invoice_items` AS `sales_items` ON sales_items.sales_invoice_id = sales_invoice.sales_invoice_id 
			LEFT JOIN `erp_dealer_master` AS `customer` ON customer.dealer_id=sales_invoice.dealer_id 
			LEFT JOIN `finance_receipt_voucher` AS `receipt` ON receipt.invoice_id=sales_invoice.sales_invoice_id and receipt.payment_by = '1' WHERE sales_invoice.invoice_status !=2 ".$where." GROUP BY sales_items.sales_invoice_id		
			UNION 		
			SELECT invoice.purchase_invoice_id, DATE_FORMAT(invoice.invoice_date,'%Y-%m-%d') as date, invoice.grand_total_amount AS `paid_amount`, `vendor`.`vendor_name`,  invoice.payment_status as payment_status, (CASE WHEN `invoice`.`payment_status` = '5' THEN 'PI' END) as payment_type, (invoice.purchase_invoice_id) as invoice_increment_id, account.account_id,account.account_name, null, null, null FROM `erp_purchase_invoice` AS `invoice` 
			...
			WHERE purchase_invoice_status !=2 ".$where1." GROUP BY invoice.purchase_invoice_id
			UNION 
			SELECT invoice.purchase_invoice_id, DATE_FORMAT(invoice.invoice_date,'%Y-%m-%d') as date, (purchase_items.purchase_invoice_item_price * purchase_items.purchase_invoice_item_approved_quantity) AS `paid_amount`, `vendor`.`vendor_name`, (CASE WHEN purchase_items.account_id != '' THEN '4' END) as payment_status, (CASE WHEN `invoice`.`payment_status` = '5' THEN 'PI' END) as payment_type, (invoice.purchase_invoice_id) as invoice_increment_id, account.account_id,account.account_name, null, null, null FROM `erp_purchase_invoice` AS `invoice` 
			LEFT JOIN `erp_vendor_master` AS `vendor` ON vendor.vendor_id=invoice.vendor_id 
			LEFT JOIN `finance_payment_vouvher` AS `payment` ON payment.invoice_id=invoice.purchase_invoice_id and payment.payment_by = '1' 
			LEFT JOIN `erp_purchse_invoice_items` AS `purchase_items` ON purchase_items.purchase_invoice_no = invoice.purchase_invoice_id
			LEFT JOIN `account_master` AS `account` ON account.account_id=purchase_items.account_id 
			WHERE purchase_invoice_status !=2 and purchase_items.account_id !='' ".$where8." GROUP BY invoice.purchase_invoice_id
			
			UNION
			SELECT finance_payment_vouvher.`payment_voucher_id`, DATE_FORMAT(finance_payment_vouvher.add_date,'%Y-%m-%d') as date, paid_amount, party_names as vendor_name, payment_status, (CASE WHEN payment_status = '4' THEN 'PV' END) as payment_type,  remarks as invoice_increment_id, null, null, null, null, null FROM `finance_payment_vouvher` left join erp_vendor_master on erp_vendor_master.vendor_id=finance_payment_vouvher.vendor_id WHERE `status` !='2' ".$where3."
			
			UNION 
			
			SELECT finance_receipt_voucher.`receipt_voucher_id`, DATE_FORMAT(finance_receipt_voucher.add_date,'%Y-%m-%d') as date, `paid_amount`,  party_names as vendor_name, `receipt_status` as payment_status, (CASE WHEN receipt_status = '5' THEN 'RV' END) as payment_type, remarks as invoice_increment_id, null, null, null, null, null FROM `finance_receipt_voucher` left join erp_dealer_master on erp_dealer_master.dealer_id=finance_receipt_voucher.dealer_id WHERE `status` !='2' ".$where2." 
	...
	public function getAccountStatementByDatesForPurchase($data)
	{
		...
		if( !empty($data['party_id']) ){		
			...
			if(!empty( $data['party_id'] )){				
				...
				$where1 .= " and vendor.vendor_id ='".$data['search_id']."' and vendor_search_type='".$data['search_type']."'";
				$where6 .= " and erp_vendor_master.vendor_id ='".$data['search_id']."' and vendor_search_type='".$data['search_type']."'";
				//$where7 .= " OR account.account_id ='".$data['search_id']."' and account_search_type='".$data['search_type']."'";
			}
			if(!empty( $data['start_date'] )){				
				$where1 .= " and invoice_date >='".$data['start_date']."' and invoice_date <='".$data['end_date']."'";				
				$where6 .= " and invoice_due_date >='".$data['start_date']."' and invoice_due_date <='".$data['end_date']."'";
	...
	public function getAccountStatementByDatesForPurchaseVat($data)
	{
		...
		if( !empty($data['party_id']) ){		
			...
			if(!empty( $data['party_id'] )){
				$where .= " and customer.dealer_id ='".$data['search_id']."' and cus_search_type='".$data['search_type']."'";
				$where1 .= " and vendor.vendor_id ='".$data['search_id']."' and vendor_search_type='".$data['search_type']."'";
				$where2 .= " OR account.account_id ='".$data['search_id']."' and account_search_type='".$data['search_type']."'";				
			}
			if(!empty( $data['start_date'] )){
				$where .= " and invoice_date >='".$data['start_date']."' and invoice_date <='".$data['end_date']."'";
				$where1 .= " and invoice_date >='".$data['start_date']."' and invoice_date <='".$data['end_date']."'";	
			...
			$select ="SELECT invoice.bill_no as sales_invoice_id, DATE_FORMAT(invoice.invoice_date,'%Y-%m-%d') as date,  sum(purchase_invoice_item_price * purchase_invoice_item_approved_quantity ) AS `paid_amount`, ( IF(invoice.operator1 = '1', invoice.price, 0) ) AS add_amount, ( IF(invoice.operator1 = '2', invoice.price, 0) ) AS sub_amount, `vendor`.`vendor_name`, `invoice`.`payment_status`,`invoice`.`price`, (CASE WHEN `invoice`.`payment_status` = '5' THEN 'PI' END) as payment_type, (invoice.purchase_invoice_id) as invoice_increment_id, account.account_id,account.account_name, null, null FROM `erp_purchase_invoice` AS `invoice` 
			LEFT JOIN `erp_purchse_invoice_items` AS `purchase_items` ON purchase_items.purchase_invoice_no = invoice.purchase_invoice_id 
			LEFT JOIN `account_master` AS `account` ON account.account_id=purchase_items.account_id 
			LEFT JOIN `erp_vendor_master` AS `vendor` ON vendor.vendor_id=invoice.vendor_id 
			WHERE purchase_invoice_status !=2 ".$where1.$where2." GROUP BY purchase_items.purchase_invoice_no			
			UNION			
	...
	public function getAccountStatementByDatesForCash($data)
	{
		...
		if( !empty($data['party_id']) ){		
			...
			$select ="SELECT finance_payment_vouvher.`payment_voucher_id` as sales_invoice_id, DATE_FORMAT(finance_payment_vouvher.add_date,'%Y-%m-%d') as date, paid_amount, (sub_amount) as add_amount, sub_amount, party_names as vendor_name, payment_status, (CASE WHEN payment_status = '4' THEN 'PV' END) as payment_type,  remarks as invoice_increment_id, null, null, null, null FROM `finance_payment_vouvher` left join erp_vendor_master on erp_vendor_master.vendor_id=finance_payment_vouvher.vendor_id WHERE `status` !='2' and payment_by='1' ".$where3."
			
			UNION 
			
			SELECT finance_receipt_voucher.`receipt_voucher_id` as sales_invoice_id, DATE_FORMAT(finance_receipt_voucher.add_date,'%Y-%m-%d') as date, `paid_amount`, (sub_amount) as add_amount, sub_amount, party_names as vendor_name, `receipt_status` as payment_status, (CASE WHEN receipt_status = '5' THEN 'RV' END) as payment_type, remarks as invoice_increment_id, null, null, null, null FROM `finance_receipt_voucher` left join erp_dealer_master on erp_dealer_master.dealer_id=finance_receipt_voucher.dealer_id WHERE `status` !='2' and payment_by='1' ".$where2."
