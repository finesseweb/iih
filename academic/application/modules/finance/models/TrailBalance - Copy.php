...
class Finance_Model_TrailBalance extends Zend_Db_Table_Abstract {
	...
	{	
		...
		LEFT JOIN `finance_receipt_voucher` AS `receipt` ON receipt.invoice_id=sales_invoice.sales_invoice_id and receipt.payment_by = '1' 
		...
		UNION 		
		SELECT invoice.purchase_invoice_id, DATE_FORMAT(invoice.invoice_date,'%Y-%m-%d') as date, sum(invoice.grand_total_amount) AS `paid_amount`, `vendor`.`vendor_name`, `invoice`.`payment_status`, (CASE WHEN `invoice`.`payment_status` = '5' THEN 'PI' END) as payment_type, (invoice.purchase_invoice_id) as invoice_increment_id, null, null, null, null,null, null, payment.payment_by FROM `erp_purchase_invoice` AS `invoice` LEFT JOIN `erp_vendor_master` AS `vendor` ON vendor.vendor_id=invoice.vendor_id LEFT JOIN `finance_payment_vouvher` AS `payment` ON payment.invoice_id=invoice.purchase_invoice_id and payment.payment_by = '1' WHERE purchase_invoice_status !=2 GROUP BY invoice.vendor_id
		UNION
		SELECT finance_payment_vouvher.`payment_voucher_id`, DATE_FORMAT(finance_payment_vouvher.add_date,'%Y-%m-%d') as date, sum(paid_amount) as paid_amount, erp_vendor_master.vendor_name, payment_status, (CASE WHEN payment_status = '4' THEN 'PV' END) as payment_type, remarks as invoice_increment_id, null,null, null, null, null, null, null FROM `finance_payment_vouvher` left join erp_vendor_master on erp_vendor_master.vendor_id=finance_payment_vouvher.vendor_id WHERE `status` !='2' and payment_by !='1'
		
		UNION 
		
		SELECT finance_receipt_voucher.`receipt_voucher_id`, DATE_FORMAT(finance_receipt_voucher.add_date,'%Y-%m-%d') as date, sum(paid_amount) as paid_amount,  erp_dealer_master.dealer_company_name as vendor_name, `receipt_status` as payment_status, (CASE WHEN receipt_status = '5' THEN 'RV' END) as payment_type, remarks as invoice_increment_id, null,null, null, null, null, null, null FROM `finance_receipt_voucher` left join erp_dealer_master on erp_dealer_master.dealer_id=finance_receipt_voucher.dealer_id WHERE `status` !='2' and payment_by !='1'
		
		UNION

		SELECT `id`, DATE_FORMAT(date,'%Y-%m-%d') as date, sum(credit_amount) as paid_amount, `dealer_id` as vendor_name, (CASE WHEN `payment_type` = '5' THEN '4' END) as payment_status, payment_type, remarks as invoice_increment_id, null, null,null, null, null, null, null FROM `journal_voucher` WHERE status !=2 and payment_type = '5' GROUP BY dealer_id

		UNION

		SELECT `id`, DATE_FORMAT(date,'%Y-%m-%d') as date, sum(debit_amount) as paid_amount, `vendor_id` as vendor_name, (CASE WHEN `payment_type` = '5' THEN '5' END) as payment_status, payment_type, remark1 as invoice_increment_id, null,null, null, null, null, null, null FROM `journal_voucher` WHERE status !=2 and payment_type = '5' GROUP BY vendor_id
		
		UNION
		
		SELECT invoice.`commercial_invoice_id`, DATE_FORMAT(invoice.invoice_due_date,'%Y-%m-%d') as date, sum(invoice.amount_inr) as paid_amount, erp_vendor_master.vendor_name, invoice.payment_status, (CASE WHEN invoice.payment_status = '5' THEN 'PCI' END) as payment_type, invoice.commercial_invoice_id as invoice_increment_id, null, null, null, null,null, null, payment.payment_by FROM `erp_purchase_commercial_invoice` as invoice left join erp_vendor_master on erp_vendor_master.vendor_id=invoice.vendor_id LEFT JOIN `finance_payment_vouvher` AS `payment` ON payment.commercial_invoice_id=invoice.commercial_invoice_id and payment.payment_by = '1' WHERE `commercial_invoice_status` !='2' group by invoice.vendor_id		
		UNION		
		SELECT erp_sales_waybill.`waybill_id`, DATE_FORMAT(erp_sales_waybill.waybill_date,'%Y-%m-%d') as date, sum(erp_sales_waybill.total_amount) as paid_amount, erp_dealer_master.custom_name as vendor_name, erp_sales_waybill.`receipt_status` as payment_status, (CASE WHEN erp_sales_waybill.receipt_status = '4' THEN 'WB' END) as payment_type, null, null, null, null,null, null, null, receipt.payment_by FROM `erp_sales_waybill` left join erp_dealer_master on erp_dealer_master.dealer_id=erp_sales_waybill.sales_dealer_id LEFT JOIN `finance_receipt_voucher` AS `receipt` ON receipt.waybill_id=erp_sales_waybill.waybill_id and receipt.payment_by = '1' WHERE `waybill_status` !='2' group by erp_sales_waybill.sales_dealer_id
