...
class Finance_Model_TrailBalance extends Zend_Db_Table_Abstract {
	...
	{
		$select ="SELECT vendor.vendor_id, vendor.vendor_name AS name, vendor.start_balance, vendor.operator, (SELECT sum(cinvoice.amount_inr) - sum(IF(payment.payment_by = '1', cinvoice.amount_inr, 0)) as amount_inr FROM erp_purchase_commercial_invoice as cinvoice LEFT JOIN finance_payment_vouvher AS payment ON payment.commercial_invoice_id=cinvoice.commercial_invoice_id LEFT JOIN erp_vendor_master ON erp_vendor_master.vendor_id=invoice.vendor_id WHERE cinvoice.vendor_id = vendor.vendor_id and cinvoice.commercial_invoice_status !=2 and cinvoice.invoice_due_date >='".$data['start_date']."' and cinvoice.invoice_due_date <='".$data['end_date']."' GROUP BY cinvoice.vendor_id) AS pcm_amount, 
