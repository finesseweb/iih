...
class Invoice_PurchaseController extends My_MasterController {
    ...
    public function indexAction() {
        ...
        switch ($type) {
            case "add":
                ...
                if ($this->getRequest()->isPost()) {
                    if ($ErpPurchaseInvoice_form->isValid($this->getRequest()->getPost())) {
                        $data = $ErpPurchaseInvoice_form->getValues();
							...
							$entry_data['inventory_grn_id'] = $data['inventory_grn_id'];
							$entry_data['vendor_id']=$this->getRequest()->getPost('vendor_id');
							if( empty($entry_data['vendor_id']) ){
								$entry_data['vendor_id']=$this->getRequest()->getPost('grn_vendor_id');
							}
						/* 	$entry_data['invoice_date']=$this->getRequest()->getPost('invoice_date');
							if( empty($entry_data['invoice_date']) ){
								$entry_data['invoice_date']=$this->getRequest()->getPost('inventory_grn_added_date');
							} */
							...
							//$item_data['vendor_id']=$purchase_grn['vendor_id'][$k];	
				...
				}elseif($data['inventory_grn_id']!='-1'){
				$purchase_grn = $this->getRequest()->getPost('purchase_grn');
				//echo '<pre>'; print_r($purchase_grn);
				foreach(array_filter($purchase_grn['inventory_grn_item_id']) as $k=>$erp_inventory_grn_item_id) 
				{
				//$item_data['purchase_grn_category_id']=$purchase_grn['purchase_grn_category_id'][$k];
				$item_data['purchase_invoice_no']=$purchase_invoice_no;
				$item_data['item_id']=$erp_inventory_grn_item_id;
				$item_data['purchase_invoice_actual_quantity']=$purchase_grn['inventory_grn_actual_quantity'][$k];
				//$item_data['vendor_id']=$purchase_grn['vendor_id'][$k];				
				$item_data['purchase_invoice_item_approved_quantity']=$purchase_grn['inventory_grn_item_approved_quantity'][$k];
				$item_data['purchase_invoice_item_rejected_quantity']=$purchase_grn['inventory_grn_item_rejected_quantity'][$k];
				$item_data['purchase_grn_item_code']=$purchase_grn['purchase_grn_item_code'][$k];
				$item_data['purchase_invoice_item_price']=$purchase_grn['inventory_grn_item_price'][$k];
				$item_data['purchase_grn_category_id']=$purchase_grn['purchase_grn_category_id'][$k];
				
				//$item_data['inventory_grn_added_date']=$purchase_quatation_data['inventory_grn_added_date'][$k];
   ...
   break;
				...
				//echo '<pre>';print_r($result);die;
				 $data = $inventory_result   = $PurchaseInvoiceItems_model->getRecord($result['purchase_invoice_id']);	
            ...
            $ErpInventoryGrn_model = new Application_Model_ErpInventoryGrn();
            $data1 = $ErpInventoryGrn_model->getDropDownListInventory();
			//print_r($data1); die;
							
				if($result['inventory_grn_id'] == '-1'){
					$data1[-1]="Direct Invoice";
				}else{
					if(strlen($result['inventory_grn_id'])==1){
					$data1[$result['inventory_grn_id']]= @(INVAL_PREFIX)."000".$result['inventory_grn_id'];
					}else
					if(strlen($result['inventory_grn_id'])==2){
					$data1[$result['inventory_grn_id']]= @(INVAL_PREFIX)."00".$result['inventory_grn_id'];
					}else
					if(strlen($result['inventory_grn_id'])==3){
					$data1[$result['inventory_grn_id']]= @(INVAL_PREFIX)."0".$result['inventory_grn_id'];
					}
					else{
					$data1[$result['inventory_grn_id']]= @(INVAL_PREFIX)."".$result['inventory_grn_id'];
					}
				}
				ksort($data1);
				$ErpPurchaseInvoice_form->getElement("inventory_grn_id")
									->setAttrib('readonly','readonly')
									->setAttrib('class',array('form-control'))
									->setMultiOptions($data1);
				$ErpPurchaseInvoice_form->populate($result);
				$PurchaseInvoice_model=new Invoice_Model_ErpPurchaseInvoiceItems();
				$purchase_inventory_items=$PurchaseInvoice_model->getRecords($result['inventory_grn_id']);
				$this->view->items=$purchase_inventory_items;
				//echo("<pre>");print_r($purchase_inventory_items);die;		
    ...
    public function getInvoiceAction() {
	 ...
	 public function getGrirInvoiceAction() {
        $this->_helper->layout->disableLayout();
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            $cmp_model = new Application_Model_ErpCompanyMaster;
            $this->view->cmpDetail = $cmp_model->getRecord(2);
           ...
           $Purchase_invoice_model = new Application_Model_ErpInventoryGrn();
		   $result = $Purchase_invoice_model->getGrnRecord($grn_id);
		  // echo"<pre>";print_r($result); die;
		   $Purchase_invoice_items_model = new Application_Model_ErpInventoryGrnItems();
		  // $item_Result = $Purchase_invoice_items_model->getGrnItemsRecords($grn_id);
		    $item_Result = $Purchase_invoice_items_model->getGrnItemsRecords($result['inventory_grn_id']);
