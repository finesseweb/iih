...
class Application_Form_UserCollectionReport extends Zend_Form {
    ...
    //    $data = $session_year->getDropDownList();
      ...
      //  $session_id = $this->createElement('select', 'session')
       ...
       // $filter_id = $this->createElement('select', 'filter_status')
        ...
        //        ->setAttrib('class', array('form-control', 'chosen-select'))
        //        ->setAttrib('required', 'required')->setRequired(true)
        //        ->addMultiOptions(array('' => 'Select Filter'))
        //        ->addMultiOptions(array('1' => 'Discount'))
        //        ->addMultiOptions(array('2' => 'Extra Charge'))
                //->addMultiOptions(array('3' => 'All Student List'))
         //       ->removeDecorator("htmlTag");
       // $this->addElement($filter_id);
		
		
		 $fee_histroy = new Application_Model_FeeHistroy();
		  $data = $fee_histroy->getDropDownList();
           $emplid = $this->createElement('select', 'empl_id')
               ->removeDecorator('label')
               ->setAttrib('class', array('form-control', 'chosen-select'))
               ->setAttrib('required', 'required')->setRequired(true)
                ->addMultiOptions(array('' => 'Select User'))
                ->addMultiOptions($data)
                ->removeDecorator("htmlTag");
        $this->addElement($emplid);
    }
