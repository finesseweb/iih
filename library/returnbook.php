...
if ($Mode == 'ADD_ITEM' || $Mode == 'UPDATE_ITEM') {
		...
		if ($Mode == 'ADD_ITEM') {
			...
			//if ($_POST['ref_id']) {
				...
				if($_POST['book_condition']==1){
					$b_con=0;
				}else{
					$b_con=$_POST['book_condition'];
				}
				if($_POST['status']==1){
					$status23=0;
				    return_book($selected_id, $status,$_POST['book_condition'],$_POST['copies_no']);
				    updatebookstatus($_POST['ISBN'],$_POST['copies_no'],$status23,$b_con);
				    $Mode = 'RESET';
					$update_pager = true;
...
kv_check_row(_("Returned Book List:"), 'returned', null, false, false);
...
start_form(true);
	...
	if ($returned =='') {
		$sql ="SELECT * FROM user_holdbook WHERE status=1";
	}else if($returned ==1){
...
$cols = array(
            ...
            _("Copy No") => array('align' => 'center','fun'=>'copy_no'),
	    ...
	    _("Return On") => array('align' => 'center','fun' => 'returnon'),
...
function copy_no($row) {
	
    return $row['copies_no'];
}
...
function returnOn($row) {
	if($row['returnon']){
		$date=date('d-m-Y', strtotime($row['returnon']));
	}else{
		$date='----';
	}
    return $date;
...
custom_list_row(_("Book Condition"), 'book_condition', null, TRUE, false, 'return');
end_table(1);
