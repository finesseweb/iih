...
page(_($help_context = "Manage Book Shelf"), @$_REQUEST['popup'], false, "", $js);
...
function can_process() {
 ...
 //    }
    ...
    if (strlen($_POST['self_code']) == 0) {
        display_error(_("Self  can not be empty."));
        set_focus('self_code');
        return false;
    }
	...
	if(can_process()){
		...
		if ($Mode == 'ADD_ITEM') {
			$result = '';
                 ...
                 // $sql = "INSERT INTO " . TB_PREF . "book_map (cat_id,ISBN, copies_no,title,author, floor, aisel,self) VALUES (" . db_escape($_POST['category']) . "," . db_escape($_POST['isbn']) . "," . db_escape(implode(",",$_POST['copiesno']))."," . db_escape($_POST['title'])."," . db_escape($_POST['author'])."," . db_escape($_POST['fl_num'])."," . db_escape($_POST['aisle_code'])."," . db_escape($_POST['self_code']).")";
...
function totalCopies($row) {
    $sql = "SELECT COUNT(*) FROM " . TB_PREF . "book_map WHERE ISBN=" . db_escape($row['ISBN'])." AND floor=". db_escape($row['floor'])." AND aisel=". db_escape($row['aisel'])." AND self=". db_escape($row['self']);
...
if($Mode == 'Edit' || check_value('show_inactive')){
...
if($_POST['aisle_code'] !=""){
 total_self_list_row2(_("Shelf Name.:"), 'self_code', null, false, true);
}
...
end_table(1);
   ...
   start_table(TABLESTYLE);
    ...
    select_book_copies($th);
    table_header($th);

    $k = 0;
   
        $result = get_ALLbookCopiesNumber23(check_value('show_inactive'),$_POST['isbn'],$_POST['fl_num'],$_POST['aisle_code'],$_POST['self_code']);
        while ($myrow = db_fetch($result)) {
            ...
            label_cell($myrow["copies_no"]);
             ...
             $sql="SELECT copies_no FROM ".TB_PREF."book_map Where copies_no = ".db_escape($myrow["copies_no"])." AND ISBN=".db_escape($myrow["ISBN"]);
            $data=db_query($sql, "Can't update record status");
            $data2=db_fetch($data);
            if($data2['copies_no']==""){
                $myrow["status"]=0;
            }else{
            ...
            update_books_copies_Number33($myrow["copies_no"], $myrow["status"], 'books_copies', 'copies_no',$myrow["ISBN"]);
            end_row();
        }

        inactive_control_row_copies($th);
        select_book_copies($th);
