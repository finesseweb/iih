...
page(_($help_context = "Max. Books on Hold"));
...
function can_process() {
	...
	if (strlen($_POST['hold_book']) == 0) {
		display_error(_("Hold Book can not be empty."));
		set_focus('hold_book');
		return false;
	}
	if (!empty($_POST['hold_book'])) {
		$regex = "/[^ ][ *@$%#^&!~()+\><{};'?<>]$/";

		if (preg_match($regex, $_POST['hold_book']) != 0) {

			display_error(_("Special characters not allowed in hold book ."));
			set_focus('hold_book');
			return false;
		}
	}
...
if ($Mode == 'ADD_ITEM' || $Mode == 'UPDATE_ITEM') {
	if(can_process()){
		...
		if ($Mode == 'ADD_ITEM') {
			$result = '';
                            add_holdBook($_POST['ref_id'], $_POST['hold_book'], $_POST['no_book'], $_POST['no_day']);
			...
			if ($_POST['ref_id']) {
				$result = '';
			    update_holdBook ($selected_id, $_POST['ref_id'], $_POST['hold_book'], $_POST['no_book'], $_POST['no_day']);
...
function holdBook($row) {
    return $row['hold_book'];
}
function bookNo($row) {
    return $row['no_book'];
}
...
if ($selected_id != -1) {
    ...
    if ($Mode == 'Edit') {
        ...
        $myrow = get_holdBook_id($selected_id);
        $_POST['ref_id'] = $myrow["ref_id"];
        $_POST['hold_book'] = $myrow["hold_book"];
        $_POST['no_book'] = $myrow["no_book"];
...
text_row(_('Hold Book for*'), 'hold_book', $_POST['hold_book'], 30,50);
room_no_row_ex(_("No. of Books *"), 'no_book', 30, null, null, null, null, null, FALSE);
