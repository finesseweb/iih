		...
		if ($Mode == 'ADD_ITEM') {
			...
			set_global_connection(0);
			    ...
			    if($_POST['ext_date']){
			         ...
			         ext_holdbook($selected_id, $_POST['ext_date']);
...
set_global_connection(0);
	$sql ="SELECT * FROM user_holdbook WHERE status !=2 and bookIssueDate!='0000-00-00'";
...
if ($selected_id != -1) {
	...
	if($myrow['empl_id']!=''){
		...
		$holReq_Data = getEXTBookList($myrow['user_id']);
                
                $get_holdDate2 =getext($_POST['s_id']);
                $re_date1= $get_holdDate2['no_time'];
	        $re_date11= $get_holdDate2['no_day'];
                if(count($holReq_Data)==$re_date1){
                    display_error(_("You had already used your extension limit"));
                }
                
                $_POST['ext_date'] = date('Y-m-d', strtotime($myrow["bookReturndDate"]. ' + '.$re_date11.' days'));
		//set_global_connection(0);
...
if ($Mode == 'Edit') {
	...
	room_no_row_ex3(_("ISBN"), 'ISBN', 30, null, null, null, null, null, FALSE);
        ...
        room_no_row_ex3(_("Extended Date"), 'ext_date', 30, null, null, null, null, null, FALSE);
        if($_POST['status1']==0){
