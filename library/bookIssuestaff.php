...
page(_($help_context = "Books Issued (Staff)"), @$_REQUEST['popup'], false, "", $js);
			...
			$direct_issue=0;  
		             ...
		             $max_number = getmaxissue();
	                      ...
	                      $bookIssueId = 'IssueId-' . $emp_inc_id;
                               ...
                               $sid="No-001";
                                ...
                                $u_Issuebook = getIssueBookList3($_POST['emp_id']);
				...
				$date->format('Y-m-d');
			        $issue_date=date('Y-m-d', strtotime("+".$re_date." days"));
                              // display_error("hello".$user['real_name']);
//                            display_error($_POST);
                              //die();
                               set_global_connection(0);
				$result = '';
			    update_issuebooks1 ($_POST['emp_id'],$user['empl_id'],$user['real_name'],$_POST['ISBN'], $_POST['title'], $_POST['copies_no'], $_POST['author'], $_POST['publisher'],$_POST['edition'],$_POST['hold_date'], $bookIssueId, $direct_issue,$issue_date);
...
if ($selected_id != -1) {
      $Mode = 'Edit';
    if ($Mode == 'Edit') {
       //$cpoies=  getcopies($_POST['ISBN']);
        //display_error($cpoies);
	//print_r($cpoies);
        //die();
...
if($cpoies=='') {
    display_error(_("Copy Of Choosen Book is Not Avialable"));
    die();
}
...
table_section_title(_("Issue Book "));
 ...
 $u_Issuebook = getIssueBookList3($_POST['emp_id']);
staff_list_cells(_("Staff:"), 'emp_id', $_POST['emp_id'], '--Select--', true);    
...
//book_category_list_cells12(_("Category:"), 'category', $_POST['category'], false, false);
