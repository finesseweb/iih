...
start_form(true);
	...
	//$sql = "SELECT * from user_holdbook";
	$sql ="SELECT * FROM user_holdbook WHERE 1=1";
...
function username($row) {
	if($row['empl_id']!=''){
		...
		$sql1 = "SELECT * FROM fa_users WHERE id=".$row['user_id'];
...
if ($selected_id != -1) {
	...
	if($myrow['empl_id']!=''){
		set_global_connection(0);
		$sql1 = "SELECT * FROM fa_users WHERE id=".$myrow['user_id'];
		$user_data = db_query($sql1);
		$user_result = db_fetch_row($user_data);
