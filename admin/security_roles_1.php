...
page(_($help_context = "Academic Access setup"));
...
start_table(TABLESTYLE2);
	text_row(_("Role name:"), 'name', null, 20, 22);
        // employment_list_row(_("Employee Type:"), 'empl_type', explode(';',$_POST['empl_type']),false,true);
	text_row(_("Role description:"), 'description', null, 50, 52);
	record_status_list_row(_("Current status:"), 'active');
end_table(1);
	...
	foreach(sort_areas($security_areas) as $area =>$parms ) {
		...
		{ // features set selection
			...
			$sec = $newsec;
                         $m = $parms[0] & ~0xff;
                 if(!in_array($security_sections[$m],$acad_setup)){hidden('Section'.$m);}else{
		...
		if (check_value('Section'.$m)) {
                    if(!in_array($security_sections[$m],$acad_setup)){hidden('Area'.$parms[0]);}else{  
