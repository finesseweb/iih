...
$slip_id = isset($_SESSION['user']['slip_id'])?$_SESSION['user']['slip_id']:'';
$employee_id = isset($_SESSION['user']['empl_id'])?$_SESSION['user']['empl_id']:'';
$month = isset($_SESSION['user']['month'])?$_SESSION['user']['month']:'';
$year = isset($_SESSION['user']['year'])?$_SESSION['user']['year']:'';
$is_arrear = isset($_SESSION['user']['arrear'])?$_SESSION['user']['arrear']:'';
...
//display_error($months_with_years_list[get_post('month')]);
 	...
 	$month_name = kv_month_name_by_id(get_post('month'));
		...
		//if(isset($employee_id) && $employee_id != '' && date('m',strtotime($months_with_years_list[get_post('month')])) >= date('m', strtotime($joining_date)) ) {
                if(isset($employee_id) && $employee_id != '' && date("Y-m",strtotime($months_with_years_list[get_post('month')])) >= date("Y-m",strtotime(get_employee_join_date($empl_id))))
                {
		...
		label_row(_("EPF (Employer's Cont.):"), price_format($_POST[12]));//New
                label_row(_("NPS-SAS (8.33% - Employer's Cont.)"), price_format($_POST[6]));//New
		...
		while ($single = db_fetch($Allowance)) {
			//label_row(_($single['description']), price_format(round((($_POST[$single['id']])*$working_days)/($total_days))), null, 30, 30);
                ...
                label_row(_("EPF (Employer's Cont.):"),  price_format($_POST[12]), null, 30, 30);
                label_row(_("NPS-SAS (8.33% - Employer's Cont.):"), price_format($_POST[6]), null, 30, 30);
