...
if (!@$_GET['popup'])
{
	...
	page(("Employees Inquiry"), @$_GET['popup'], false, "", $js);
}
...
ref_cells(_("Employee Id:"), 'empl_id', '',null, _('Enter Employee id fragment or leave empty'));
ref_cells(_("Employee Name:"), 'empl_name', '',null, _('Enter Employee Name fragment or leave empty'));
ref_cells(_("Email Id:"), 'empl_email', '',null, _('Enter Email ID fragment or leave empty'));
departments_list_cells(_("Department:"), 'dept_id', null, false, false);
hrm_empl_status_list_search(_("Status:"), 'status', null,true);
...
function display_rows(){
	$sql = kv_hrm_get_empl_list( get_post('empl_id'),get_post('empl_name'),get_post('empl_email'), get_post('dept_id', -1),get_post('status', -1));
	...
	$cols = array(
		...
		_("Contract End Date") => array('name'=>'contract_end_date')
		//array('')
	);	
