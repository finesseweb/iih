...
<td><?php label_cell(pager_link( _("Edit"),
		"/modules/ExtendedHRM/inquires/leaves_request_report.php?id=".$myrow["request_status"], ICON_EDIT)); ?>
</td>
<?php } ?>
...
<td><?php echo 'Submitted'; ?></td>
<td><?php label_cell(pager_link( _("Edit"),
		"/modules/ExtendedHRM/inquires/leaves_request_report.php?id=".$myrow["request_status"], ICON_EDIT)); ?></td> 
<?php } ?>
...
if (isset($_GET['id']))
{
	...
	$query3 = "SELECT reqst.* FROM ".TB_PREF."kv_allocation_request_status AS reqst  WHERE reqst.status_id=".db_escape($_GET['id'])."";
	//display_error($query3);
	$result3= db_query($query3);
	...
	//table_header($th);
		...
		label_row(_("Employee Name"),$myrow3["employees_id"]);
		...
		textarea_row(_("Comments :"),$_POST["comments"]);
		?>
	<!-- 	<td>Comments : <textarea id="comments<?php // echo $i;?>" style=""><?php // echo $_POST['comments'] ; ?></textarea></td>  -->
	<?php 	//display_error($_POST['approved_leaves']);
		end_table();
     end_table();
	}
}
