...
page(_($help_context = "Compensatory Approval"), @$_REQUEST['popup'], false, "", $js);
?>
<html lang="en">
    <head>
...
function display_employee_leave_status_records($selected_parent, $dept_id, $employee_id, $access_id, $from_date, $to_date) {
    ...
    $result = get_employee_leave_status_records_comp($selected_parent, $dept_id, $employee_id, $access_id, $from_date, $to_date);
   
    div_start('bom');
    start_table(TABLESTYLE, "width='60%'");
    $th = array(_("Employee Name"), _("Leave Type"), _("Reason"),
        _("Requested From Date"), _("Requested To Date"), _("No. of Days"), _("Status"), _("No. of Days Approved"), _("Approved From Date"), _("Approved To Date"), _("Comments"), _("working Hrs."),'', '', '');
    table_header($th);
...
function on_edit($selected_parent, $selected_component = -1) {
  ...
  update_leave_request_comp($selected_component, $_POST['leave_status_type'], $_POST['no_of_days_approved'], $approved_from_date, $approved_to_date, $_POST['comments']);
    //update_allocation($selected_component,$_POST['dept_id'],$_POST['desig_group_id'],$_POST['desig_id'],$selected_parent,$_POST['type_leave'],$_POST['reason'],$today_date,$from_date,$to_date,$_POST['no_of_days'],$_POST['upload_file'],$_POST['filesize'],$_POST['filetype'],$_POST['unique_name'],$request_date);
               $diff_mon = date_diff2($approved_from_date, $approved_to_date, 'd');
    ...
    if($_POST['leave_status_type'] == 2){ //=======[if Leave is approved]
                            for($i=0;$i<(int)$diff_between_start_end_dates; $i++){
                                              ...
                                              $month = date("m", $attendance_date);
                                               ...
                                               $year = get_fiscal_year_id_from_date(date("Y-m-d",$attendance_date));
                                                $holiday_date =  get_holiday_check(date("Y-m-d",$attendance_date), date("Y-m-d",$attendance_date), $fisical['id']);
                                                 $logindetails = get_in_out_time_by_date($empl_id, date("Y-m-d",$attendance_date));
                                                  if(in_array($weekday, explode(',',$woarr['weekly_off'])) && $logindetails[0]){
                                                    update_employee_attendance('WOP', $empl_id, $month, $year, $day);
                                                  }
                                                else if ($holiday_date && $logindetails[0]){
                                                 update_employee_attendance('HP', $empl_id, $month, $year, $day);
                                                } 
       ...
       for($i=0;$i<(int)$diff_between_start_end_dates; $i++){
                                              ...
                                              $month = date("m", $attendance_date);
                                               ...
                                               $year = get_fiscal_year_id_from_date(date("Y-m-d",$attendance_date));
                                                $holiday_date =  get_holiday_check(date("Y-m-d",$attendance_date), date("Y-m-d",$attendance_date), $fisical['id']);
                                                 $logindetails = get_in_out_time_by_date($empl_id, date("Y-m-d",$attendance_date));
                                                 $code = in_array($weekday, explode(',',$woarr['weekly_off']))?"WO":"H";
                                                  if(in_array($weekday, explode(',',$woarr['weekly_off'])) && $logindetails[0]){
...
//Parent Item selected so display bom or edit component
    ...
    if ($selected_id != -1) {
        ...
        if ($Mode == 'Edit') {
            ...
            $_POST['type_leave'] = "Compensatory Leave"; // by Tom Moulton
    ...
    label_row(_("Leave Type:"), "Compensatory Leave");
