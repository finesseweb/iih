...
if ($Mode == 'ADD_ITEM' || $Mode == 'UPDATE_ITEM') {
	if(can_process()){
		...
		if ($Mode == 'ADD_ITEM') {
			$result = '';
            add_store($_POST['location'], $_POST['store'], $_POST['store_desc']);
			...
			$result = '';
		    update_store($selected_id, $_POST['location'],$_POST['store'],$_POST['store_desc']);
...
if ($selected_id != -1) {
    ...
    if ($Mode == 'Edit') {
        ...
        $myrow = get_storeLocation_id($selected_id);
        $_POST['location'] = $myrow["location"];
        $_POST['store_desc'] = $myrow["store_desc"];
        $_POST['store'] = $myrow["store"];
    }
...
room_no_row_ex(_("Store*"), 'store', 30, null, null, null, null, null, FALSE);
room_no_row_ex(_("Store Description *"), 'store_desc', 30, null, null, null, null, null, FALSE);
end_table(1);
