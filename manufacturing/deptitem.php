...
page(_($help_context = "Issue Items to Department"), false, false, "", $js);
...
{
	$id = $_GET['AddedID'];
   	display_notification(_("The Department Item has been Issued."));
...
if (isset($_POST['addissue']) && can_process())
{
	...
	$failed_data = add_dept_issues($_POST['department'],
		$_POST['ref'], 0, $_SESSION['issue_items']->line_items,
		$_POST['Location'], $_POST['WorkCentre'], $_POST['date_'], $_POST['memo_'],$_POST['sl_no'],"D");
	...
	{
		meta_forward($_SERVER['PHP_SELF'], "AddedID=".$_POST['department']);
	}
...
start_form();
display_search_details('D');

div_start('controls', 'items_table');
display_dept_requirements($_POST['department'],'');
div_end();
$Ajax->activate('items_table');
echo "<br>";
