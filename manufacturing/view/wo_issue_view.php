...
function display_wo_issue($issue_no)
{
    ...
    $th = array(_("Issue #"), _("Reference"), _("For Work Order #"),
    	_("Item"),_("Item Code"), _("From Location"), _("To Work Centre"), _("Date of Issue"));
    table_header($th);
...
function display_wo_issue_details($issue_no)
{
    ...
    {
        start_table(TABLESTYLE);
        $th = array(_("Component"), _("Quantity"), _("Units"), _("Unit Cost"),_('Serial No'));
        ...
        while ($myrow = db_fetch($result))
        {
			...
			amount_cell($myrow["unit_cost"]);
                        label_cell(!$myrow["sl_no"]?'NA':$myrow["sl_no"]);
			end_row();
