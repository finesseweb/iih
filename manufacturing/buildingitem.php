...
page(_($help_context = "Issue Items to Building"), false, false, "", $js);
...
{
	$id = $_GET['AddedID'];
   	display_notification(_("The building Item has been Issued."));
// display_note(get_trans_view_str(ST_BUILDING, $id, _("View this building items")));
handle_new_order();
...
if (isset($_POST['addissue']) && can_process())
{
	...
	$failed_data = add_building_issues($_POST['name'],
		$_POST['ref'], 0, $_SESSION['issue_items']->line_items,
		$_POST['Location'], $_POST['WorkCentre'], $_POST['date_'], $_POST['memo_'],$_POST['sl_no']);
	...
	{
		meta_forward($_SERVER['PHP_SELF'], "AddedID=".$_POST['name']);
	}
...
function handle_new_item()
{
	if (!check_item_data())
		return;
       display_error($_SESSION['issue_items']); 
...
display_building_requirements($_POST['name'],'');
div_end();
$Ajax->activate('items_table');
echo "<br>";
