...
if(isset($_POST['route_value'])){
	$route_value = $_POST['route_value'];
	$sql2 ="SELECT source,destination FROM ".TB_PREF."route Where route_name=".db_escape($route_value);
	$route_result2=db_query($sql2); 
	$routeVal=db_fetch($route_result2);

	echo json_encode($routeVal);
}

if(isset($_POST['route_name'])){
	$route_name = $_POST['route_name'];
	$sql2 ="SELECT s_name,stop_name FROM ".TB_PREF."routeconfig_detail JOIN ".TB_PREF."route_config ON ".TB_PREF."route_config.config_id=".TB_PREF."routeconfig_detail.config_id JOIN ".TB_PREF."stop ON ".TB_PREF."stop.id=".TB_PREF."routeconfig_detail.s_name where fa_route_config.route_name=".db_escape($route_name)." AND ".TB_PREF."stop.status=1";
