<?php
$page_security = 'SA_ROUTECONFIGDETAILS';
if (!@$_GET['popup'])
	$path_to_root = "..";
else	
	$path_to_root = "../..";
...
page(_($help_context = "Route Configuration Edit"), @$_REQUEST['popup'], false, "", $js);
...
simple_page_mode(true);
	...
	$val=get_routeResultDetails($_GET['config']);
	$count_val=get_routeResultDetails2($_GET['config']);

	////////////Get Route detail ////////////////
	$r1_sql ="SELECT * FROM ".TB_PREF."route_config where config_id=".db_escape($_GET['config']);
...
<div class="panel panel-default">
	<form class="no-margin" id="formValidate1" action="" method="post" onSubmit="return form_validate();">
		<div class="panel-body">
			<div class="row">
				<div class="col-sm-3" style="margin-top:25px;">
					...
					<div class="form-group">
						<input type="text" id="config_id" class="form-control" name="config_id" value="<?=$r1_val[1]?>" readonly/>
					</div>
				</div>
				<div class="col-sm-3" style="margin-top:25px;">
				    <label class="">Route Name</label>
					<div class="form-group">
						<input type="text" id="route_name" class="form-control" name="route_name" value="<?=$r1_val[2]?>" readonly/>

						<!-- <select id="route_name" class="form-control" name="route_name" readonly>
						  <option value="<?=$r1_val[2]?>"><?=$r1_val[2]?></option>
						  <?php
						   while ($myrow = db_fetch($route_result)){?>
			...
			<?php if($count_val > 0) {  ?>
			<input type="hidden" name="count_val" id="count_val" value="<?php echo ($count_val+1) ; ?>" class="count_val">
			...
			?>
				<div class="row">
						...
						<div class="form-group">
							...
							<div class="form-group">
								<select id="stop_name" class="form-control" name="stop_name[]">
									<?php 
										$stopNameSql ="SELECT stop_name FROM ".TB_PREF."stop where id=".db_escape($myrow['s_name']);
								        $stopNameResult=db_query($stopNameSql);
								        $stopName=db_fetch($stopNameResult);
									?>
								  <option value="<?=$myrow['s_name']?>"><?=$stopName[0]?></option>
								  <?php  
								  $stop_sql ="SELECT * FROM ".TB_PREF."stop WHERE status=1";
							      $stop_result=db_query($stop_sql);
								  while ($stop_row = db_fetch($stop_result)){?>
					...
					<div class="col-sm-2"><div class="form-group"><label class="">Sequence</label><input type="text" name="sequence[]" id="sequence" value="<?=$myrow['sequence']?>" class="form-control" /></div></div>
				
					<div class="col-sm-2"><div class="form-group"><label class="">Expected Pickup Time</label><input type="time" name="exp_time[]" id="exp_time" value="<?=$myrow['exp_time']?>" class="form-control" /></div></div>
					<div class="col-sm-2"><div class="form-group"><label class="">Drop Time</label><input type="time" name="drop_time[]" id="drop_time" value="<?=$myrow['drop_time']?>" class="form-control" /></div></div>
				
					<div class="col-sm-2"><div class="form-group"><label class="">Cost(Rs.)</label><input type="text" name="cost[]" id="cost" value="<?=$myrow['cost']?>" class="form-control" /></div></div>
					<div class="col-sm-2">
						<div class="form-group">
					...
					<a  class="removeclass btn btn-primary remove_class pull-right" style="margin:24px 15px;padding:7px 15px;" href="#">-</a>
...
/*Stop Name*/
	...
	if(isset($_POST['submit'])){
		//update_routeConfigval($config_id, $route_name);
		if($_POST['count_val'] > 1 ){
			
			delete_route($_POST['config_id']);
...
var count = $('#count_val').val(); //Addmore script code
		...
		append_html = '<div><div class="row" style="border:2px solid #ccc; padding:24px;margin-top:25px;">';
		append_html += '<div class="col-sm-2"><div class="form-group"><label class="">Stop Name</label><div class="form-group"><select id="stop_name hh" class="form-control" name="stop_name[]"><option value="">Select</option><?php while ($stop_row = db_fetch($stop_result)){?><option value="<?=$stop_row['id']?>"><?=$stop_row['stop_name']?></option><?php } ?></select></div></div></div>';
