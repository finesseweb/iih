...
if (get_post('AddPaymentItem') && can_process()) {
	...
	$payment_no = write_customer_payment($_SESSION['alloc']->trans_no, $_POST['customer_id'], $_POST['BranchID'],
		...
		input_num('amount'), input_num('discount'), $_POST['memo_'], 0, input_num('charge'), input_num('bank_amount', input_num('amount')),$_POST['gst'],input_num('gst_amt'),input_num('cst'),input_num('cst_amt'),input_num('ist'),input_num('ist_amt'),input_num('hsn_no'),input_num('currency'));
...
if (isset($_GET['trans_no']) && $_GET['trans_no'] > 0 )
{
	...
	$_POST['customer_name'] = $myrow["DebtorName"];
        $_POST['gst'] = $myrow["gst"];
        $_POST['gst_amt'] = $myrow["gst_amt"];
        $_POST['cst'] = $myrow["cst"];
        $_POST['cst_amt'] = $myrow["cst_amt"];
        $_POST['ist'] = $myrow["ist"];
        $_POST['ist_amt'] = $myrow["ist_amt"];
        $_POST['hsn_no'] = $myrow["hsn_no"];
