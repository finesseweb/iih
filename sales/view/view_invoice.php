...
{
	$th = array(_("Item Code"), _("Item Description"), _("HSN No"), _("Quantity"),
		_("Unit"), _("Price"),  _("GST"), _("GST Amount"), _("CST"), _("CST Amount"), _("IST"), _("IST Amount"),_("Discount %"), _("Total"));
	table_header($th);
	...
	while ($myrow2 = db_fetch($result))
	{
		...
		$value = round2(((1 - $myrow2["discount_percent"]) * $myrow2["unit_price"] * $myrow2["quantity"] + $myrow2["unit_price"]*$myrow2["gst"]/100+$myrow2["unit_price"]*$myrow2["cst"]/100+$myrow2["unit_price"]*$myrow2["ist"]/100),
		   user_price_dec());
		$sub_total += $value;
		...
		label_cell($myrow2["StockDescription"]);
                label_cell($myrow2["hsn_no"]);
        qty_cell($myrow2["quantity"], false, get_qty_dec($myrow2["stock_id"]));
        label_cell($myrow2["units"], "align=right");
        amount_cell($myrow2["unit_price"]);
        amount_cell($myrow2["gst"]);
        amount_cell($myrow2["gst_amt"]);
        amount_cell($myrow2["cst"]);
        amount_cell($myrow2["cst_amt"]);
        amount_cell($myrow2["ist"]);
        amount_cell($myrow2["ist_amt"]);
	...
	label_row(_("Sub-total"), $display_sub_tot, "colspan=13 align=right",
...
if ($myrow['ov_freight'] != 0.0)
{
	...
	label_row(_("Shipping"), $display_freight, "colspan=13 align=right", "nowrap align=right");
}
...
label_row(_("TOTAL INVOICE"), $display_sub_tot, "colspan=13 align=right",
	"nowrap align=right");
